...
bc_land_rast <- raster(file.path(dir_spatial, 'raster/ohibc_land_raster_1000m.tif'))

wt_sum_crop <- wt_sum %>% projectRaster(bc_land_rast) %>%
# wt_sum_crop <- wt_sum_crop %>%
  crop(extent(c(1100000, 1300000, 300000, 420000)))
plastic_matrix <- rasterToPoints(wt_sum_crop)

plastic_spdf <- SpatialPointsDataFrame(plastic_matrix[, 1:2], data.frame(plastic_matrix[, 3]), 
