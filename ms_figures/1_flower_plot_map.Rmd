...
if(!file.exists(file.path(dir_figs, 'etopo_rast.tif'))) {
  ...
  etopo <- etopo_raw %>%
    ...
    projectRaster(crs = '+init=epsg:3005', res = 1000) %>%
    crop(extent(bbox))
  ...
  bcland_rast <- etopo %>%
    mask(bcland_sf %>% as('Spatial'))
  ohibc_rast <- etopo %>%
    mask(ohibc_sf %>% as('Spatial'))

  values(etopo)[values(etopo) > -5 & 
                  is.na(values(bcland_rast)) & 
                  !is.na(values(ohibc_rast))] <- -5
    ### values close (or above) zero that aren't "land" but are in OHIBC, 
    ### set to a nice depth
  ...
  crs(etopo) <- '+init=epsg:3005'
}

slope_raster <- terrain(etopo, opt = 'slope')
aspect_raster <- terrain(etopo, opt = 'aspect')
hill_raster <- hillShade(slope_raster, aspect_raster, 40, 270)
hill_df <- rasterToPoints(hill_raster) %>%
  as.data.frame() %>%
  setNames(c('x', 'y', 'hill')) %>%
  mutate(hill = ifelse(hill > .5, hill * 2 - 1, 0))
    ### just emphasize the steep hills
...
basemap <- ggplot() +
  ...
  geom_raster(data = hill_df, aes(x, y, alpha = hill), color = 'black', show.legend = FALSE) +
