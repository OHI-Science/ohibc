...
basemap <- ggplot() +
  ...
  scale_fill_gradientn(colors = basemap_cols,
                       values = basemap_vals) +
  geom_sf(data = bcland_sf, alpha = .5, color = 'grey50', fill = NA, size = .25) +
...
for(rgn in 1:8) { ### rgn <- 1
  rgn_name <- rgn_names$rgn_name[rgn_names$rgn_id == rgn]
...
x <- ggdraw() +
  ...
  draw_plot_ctr(rgn_flowers[[2]], 0.37, 0.86, sm_fl_size) +
  ...
  draw_plot_ctr(rgn_flowers[[8]], 0.75, 0.75, sm_fl_size)

ggsave('tmp.png', height = 9, width = 10, dpi = 300)
