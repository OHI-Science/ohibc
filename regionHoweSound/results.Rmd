...
# source(file.path(dir_git, 'src/R/plot_flower.R'))
# # source('https://raw.githubusercontent.com/OHI-Science/bhi/draft/baltic2015/PlotFlowerMulti.R')
# ohicore::PlotFlower(main       = 'Howe Sound',
#                     lengths    = ifelse(is.na(scores$score), 100, scores$score),
#                     widths     = scores$weight,
#                     fill.col   = ifelse(is.na(scores$cols.goals.all), 'grey80', scores$cols.goals.all),
#                     labels     = ifelse(is.na(scores$score),
#                                         paste(scores$goal, '-', sep='\n'),
#                                         paste(as.character(scores$goal), round(scores$score), sep='\n')),
#                     center     = 'X',
#                     max.length = 1)
# dev.off()
...
# plot_flower(lengths = status_df$score, widths = status_df$weight, labels = status_df$goal, max.length = 1)

status_df <- status_df %>% ### last + half(previous_width) + half(current_width)
  mutate(pos = cumsum(weight) - 0.5 * weight, #+ 0.5 * lag(weight, 1, default = 0),
         pos_end = last(pos) + 0.5 * last(weight))
  
p_labels <- status_df$goal
p_breaks <- status_df$pos
p_limits <- c(0, status_df$pos_end[1])
p_score  <- round(weighted.mean(status_df$score, status_df$weight, na.rm = TRUE) * 100, 0)

hs_flower <- ggplot(data = status_df, aes(x = pos, y = score, fill = score, width = weight)) +
  geom_bar(aes(y = 1), stat = 'identity', color = 'grey95', fill = 'grey90') +
  geom_bar(stat = 'identity', color = 'grey20', size = .2) +
  coord_polar(start = - status_df$pos[1]/status_df$pos_end[1] * 2 * pi) + 
...
ggsave(filename = file.path(dir_results, 'howe_sound_flower.png'),
