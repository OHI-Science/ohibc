...
scores_ohibc <- scores_w_index %>%
  ...
  summarize(score = weighted.mean(score, w = area_km2),
            region_id = 0)
  
scores_new_res <- scores_w_index %>%
  bind_rows(scores_ohibc)

write_csv(scores_new_res, file.path(dir_calc, 'scores_all_new_res.csv'))

```

``` {r}

scores_orig <- read_csv(file.path(dir_calc, 'scores_all.csv')) %>%
  rename(score_orig = score)
scores_new <- read_csv(file.path(dir_calc, 'scores_all_new_res.csv')) %>%
