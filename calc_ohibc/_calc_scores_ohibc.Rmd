...
message('Calculating non-governance resilience...')
### copy governance resilience csv to overwrite overall resilience matrix
### (rewriting the conf copy, not the master copy!)
file.copy(from = file.path(dir_calc, 'master', 'resilience_matrix_nonreg_master.csv'),
          to   = file.path(dir_calc, 'conf', 'resilience_matrix.csv'),
          overwrite = TRUE)
...
tmp <- read_csv(file.path(dir_calc, 'scores_all.csv')) %>%
  spread(dimension, score) %>%
  left_join(res_scores_all %>% mutate(dimension = 'res_nonreg') %>% spread(dimension, score))

write_csv(res_scores_all, file.path(dir_calc, 'res_nonreg_only.csv'))
