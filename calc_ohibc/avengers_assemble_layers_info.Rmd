...
source(file.path(dir_calc, 'create_layer_to_target_lookup.R'))
       
source(file.path(dir_calc, 'create_layer_source_lookup.R'))
...
layers_meta    <- read_csv(file.path(dir_calc, 'master/layers_meta_master.csv')) %>%
  select(-targets)
layers_source  <- read_csv(file.path(dir_calc, 'master/layers_source_master.csv'))
layers_targets <- read_csv(file.path(dir_calc, 'master/layers_targets_master.csv'))
...
write_csv(missing, file.path(dir_calc, 'xxx_missing_layers.csv'))
...
Layers in 'output' folders that are not included in the layers master list might be orphans.

``` {r identify_extra_layers}

### ID all .csv files in prep directories
all_csvs <- list.files('~/github/ohibc/prep', 
                    pattern = '.csv$',
                    recursive = TRUE, 
                    full.names = TRUE)
### narrow down to just those in output folders
output_csvs <- all_csvs[str_detect(all_csvs, '/output/') & !str_detect(all_csvs, '/vHS/')]
  
extras <- output_csvs[!basename(output_csvs) %in% layers_full$filename]
