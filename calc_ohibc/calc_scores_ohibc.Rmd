...
scenarios <- read_csv(file.path(dir_calc, 'master/data_year_matrix_master.csv')) %>%
  .$scen_name %>%
  unique()
...
layers$data$data_years <- read_csv(file.path(dir_calc, 'conf/data_year_matrix.csv'))

scores_all <- data.frame() ### initialize scores

for (scen_i in scenarios) { ### scen_i <- scenarios[1]

  message('Calculating scores for ', scen_i)

  ### For each run through loop, assign status_year inside
  ###   the layers object/env't so it is accessible to functions.R
  layers$data$scenario <- scen_i

  capture.output( ### the function prints crap to screen
    {
      scores <- CalculateAll(conf, layers) %>%
        mutate(scenario = scen_i,
               year     = as.integer(str_replace_all(scen_i, '[^0-9]', '')))
    },
    file = 'delete_this.txt')
  unlink('delete_this.txt')
