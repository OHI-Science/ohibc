...
**K-fold cross validation runs:**  We will include repeated K-fold cross validation on each (valid) run as well... based on [this helpful page](http://www.sthda.com/english/articles/38-regression-model-validation/157-cross-validation-essentials-in-r/).
...
calc_res_regr <- function(df, train.control) {
  ...
  cross_val_mdl <- train(d_prs ~ 1 + factor(year) + factor(rgn_code), data = df, method = "lm",
                         trControl = train.control)
...
for(goalname in goals) {
  ...
  for(yr_lag in 1:6) {
    ...
    tmp <- calc_res_regr(d_prs_df, train.control)
