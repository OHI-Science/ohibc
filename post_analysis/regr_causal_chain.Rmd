...
for(yr_lag in 1:6) {
  ...
  for(goalname in goals) { # goalname <- 'LSP'
    ...
    if(all(is.na(tmp_df$res_reg))) next()
...
for(yr_lag in 1:6) {
  ...
  for(goalname in goals) { # goalname <- 'LSP'
    ...
    ### Check that there are values for each component, otherwise devolves to different model:
    if(all(is.na(tmp_df$res_soc))) next()
    if(all(is.na(tmp_df$res_reg))) next()
...
for(yr_lag in 1:6) {
  ...
  for(goalname in goals) { # goalname <- 'LVO'
    ...
    tmp_mdl <- lm(d_prs ~ res_soc, data = tmp_df)
    prs_res_lm <- tmp_mdl %>%
      broom::tidy() %>%
      mutate(goal  = goalname,
             n_obs = nrow(tmp_df),
...
write_csv(prs_res, 'int/prs_res_soc_only.csv')
...
for(yr_lag in 1:6) {
  ...
  for(goalname in goals) { # goalname <- 'LSP'
    ...
    tmp_mdl <- lm(d_prs ~ res_soc + res_ecol, data = tmp_df)
    prs_res_lm <- tmp_mdl %>%
      broom::tidy() %>%
      mutate(goal  = goalname,
             n_obs = nrow(tmp_df),
...
write_csv(prs_res, 'int/prs_res_soc_ecol.csv')
...
for(yr_lag in 1:6) {
  ...
  for(goalname in goals) { # goalname <- 'LSP'
    ...
    tmp_mdl <- lm(d_prs ~ res_ecol, data = tmp_df)
    prs_res_lm <- tmp_mdl %>%
      broom::tidy() %>%
      mutate(goal  = goalname,
             n_obs = nrow(tmp_df),
...
write_csv(prs_res, 'int/prs_res_ecol_only.csv')
...
for(yr_lag in 1:6) {
  ...
  for(goalname in goals) { # goalname <- 'LSP'
    ...
    tmp_mdl <- lm(d_prs ~ 1, data = tmp_df)
    prs_res_all_lm <- tmp_mdl %>%
      broom::tidy() %>%
      mutate(goal  = goalname,
             n_obs = nrow(tmp_df),
...
write_csv(prs_res_all, 'int/prs_intercept_only.csv')
...
scores_rgn_lfs <- read_csv(file.path(dir_calc, 'scores_all.csv')) %>%
# scores_rgn <- read_csv(file.path(dir_calc, 'scores_all.csv')) %>%
  filter(region_id != 0) %>%
  spread(dimension, score) %>%
  select(goal, region_id, year, status, pressures) %>%
  left_join(prs_no_social %>%
              rename(prs_no_soc = pressures), 
...
for(yr_lag in 1:6) {
  ...
  for(goalname in goals) { # goalname <- 'LSP'
    ...
    if(nrow(tmp_df) == 0) next()
    if(all(is.na(tmp_df$prs_no_soc))) next()
    
    tmp_mdl <- lm(d_status ~ prs_no_soc, data = tmp_df)
    lfs_prs_lm <- tmp_mdl %>%
      broom::tidy() %>%
      mutate(goal  = goalname,
             n_obs = nrow(tmp_df),
...
write_csv(lfs_prs, 'int/lfs_prs_no_soc.csv')
