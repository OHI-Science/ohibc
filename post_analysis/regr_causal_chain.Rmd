...
for(yr_lag in 1:6) {
  ...
  for(goalname in goals) { # goalname <- 'LSP'
    ...
    d_prs_mdl <- lm(d_prs ~ res_reg, data = d_prs_df)
...
for(yr_lag in 1:6) {
  ...
  for(goalname in goals) { # goalname <- 'LSP'
    ...
    d_prs_mdl <- lm(d_prs ~ res_reg + res_soc, data = d_prs_df)
...
for(yr_lag in 1:6) {
  ...
  for(goalname in goals) { # goalname <- 'LSP'
    ...
    d_prs_mdl <- lm(d_prs ~ res_reg + res_ecol, data = d_prs_df)
...
for(yr_lag in 1:6) {
  ...
  for(goalname in goals) { # goalname <- 'LVO'
    ...
    d_prs_mdl <- lm(d_prs ~ res_soc, data = d_prs_df)
...
for(yr_lag in 1:6) {
  ...
  for(goalname in goals) { # goalname <- 'LSP'
    ...
    d_prs_mdl <- lm(d_prs ~ res_soc + res_ecol, data = d_prs_df)
    prs_res_lm <- tidy_model(d_prs_mdl, d_prs_df)
...
for(yr_lag in 1:6) {
  ...
  for(goalname in goals) { # goalname <- 'LSP'
    ...
    d_prs_mdl <- lm(d_prs ~ res_ecol, data = d_prs_df)
    prs_res_lm <- tidy_model(d_prs_mdl, d_prs_df)
...
for(yr_lag in 1:6) {
  ...
  for(goalname in goals) { # goalname <- 'LSP'
    ...
    d_prs_mdl <- lm(d_prs ~ res_reg + res_ecol + res_soc, data = d_prs_df)
...
for(yr_lag in 1:6) {
  ...
  for(goalname in goals) { # goalname <- 'LSP'
    ...
    d_prs_mdl <- lm(d_prs ~ 1, data = d_prs_df)
    prs_res_all_lm <- tidy_model(d_prs_mdl, d_prs_df)
...
for(yr_lag in 1:6) {
  ...
  for(goalname in goals) { # goalname <- 'FIS'
    ...
    d_status_mdl <- lm(d_status ~ pressures, data = d_status_df)
...
for(yr_lag in 1:6) {
  ...
  for(goalname in goals) { # goalname <- 'LSP'
    ...
    if(nrow(d_status_df) == 0) next()
    if(all(is.na(d_status_df$prs_no_soc))) next()
    
    d_status_mdl <- lm(d_status ~ prs_omit_soc, data = d_status_df)
    d_status_prs_lm <- tidy_model(d_status_mdl, d_status_df)
