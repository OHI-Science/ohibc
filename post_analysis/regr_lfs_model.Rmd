...
for(goalname in goals) { # goalname <- goals[13]
  ...
  lfs_lm <- lm(d_status ~ trend + res_minus_prs, data = tmp_df) %>%
    broom::tidy() %>%
    mutate(goal  = goalname,
           n_obs = nrow(tmp_df),
           years = length(tmp_df$year %>% unique()))

  lfs_res_lm <- lm(d_status ~ resilience, data = tmp_df) %>%
    broom::tidy() %>%
    mutate(goal  = goalname,
           n_obs = nrow(tmp_df),
           years = length(tmp_df$year %>% unique()))
  ...
  if(length(tmp_df$year %>% unique()) > 1) {
    lfs_felm_yr <- lm(d_status ~ trend + res_minus_prs + factor(year), 
                      data = tmp_df) %>%
      broom::tidy() %>%
      mutate(goal  = goalname,
             n_obs = nrow(tmp_df),
...
for(yr_lag in 1:6) {
  ...
  for(goalname in goals) { # goalname <- 'LSP'
    ...
    tmp_mdl <- lm(d_status ~ res_reg, data = tmp_df)
    ...
    tmp_mdl <- lm(d_status ~ res_reg + pressures, data = tmp_df)
    ...
    if(all(is.na(tmp_df$res_ecol)) | all(is.na(tmp_df$res_soc))) {
      tmp_mdl <- lm(d_status ~ trend + res_reg + res_nonreg + pressures, 
                    data = tmp_df)
    } else {
      tmp_mdl <- lm(d_status ~ trend + res_reg + res_soc + res_ecol + pressures, 
                    data = tmp_df)
    }
...
write_csv(lfs_mdl, 'int/lfs_model_compare.csv')
