...
``` {r set up BC WDPA polygons, echo = TRUE, eval = TRUE, verbose = FALSE}
...
if(nrow(report) > 0) {
  ...
  print(report %>% dplyr::select(-type, -valid, -error_msg))
...
```{r set up base raster, echo = TRUE, eval = TRUE, verbose = FALSE}
...
if(any(!inval$valid)) print(inval[!inval$valid])
