...
source('R/common.R')  ### an OHIBC specific version of common.R
...
Check for invalid geometries, and 

``` {r set up BC WDPA polygons, echo = TRUE, eval = TRUE}
...
if(nrow(report) > 0) {
  ...
  print(report_clean %>% filter(valid == FALSE))
...
``` {r set up base raster, echo = TRUE, eval = TRUE, warning = FALSE}
poly_3nm <- readShapePoly(fn = file.path(dir_rgn, 'ohibc_offshore_3nm'), proj4string = CRS(p4s_bcalb))
poly_1km <- readShapePoly(fn = file.path(dir_rgn, 'ohibc_inland_1km'),   proj4string = CRS(p4s_bcalb))
...
plot(poly_3nm,     col = rgb(.4, .5, 1, .5), border = rgb(.4, .5, 1, 1))
plot(poly_wdpa_bc, col = rgb(1, .5, .5, .5), border = NA, add = TRUE)
```

### (old: Convert inland 1 km and offshore 3nm polygons to rasters)

Note: these rasters are currently not used in final analysis... this is just for reference...
...
pull the ggplot plotting function into an OHIBC specific one...
``` {r plot scores as polygons, eval = TRUE, echo = FALSE}
source('R/poly_plot_scores.R')

yr_scores <- lsp_status %>%
  filter(year == 2015)

#poly_plot_scores(yr_scores, val = 'status')
```