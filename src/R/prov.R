...
### version of a given file.
  ...
  git_df <- data.frame('file_loc'      = as.character(git_file),
                       'filetype'      = tolower(as.character(filetype)),
...
script_prov <- function(script_file, tag = prov_run_tag, commit_outputs = TRUE) {
  ...
  script_track <<- prov_track %>%
    mutate(elapsed_time  = run_time,
           ...
           rdf_predicate = ifelse(str_detect(filetype, 'out'), 'was generated by',
                             ifelse(str_detect(filetype, 'in'), 'was used as input by',
                               ifelse(str_detect(filetype, 'source'), 'was sourced by',
...
writeRaster <- function(x, filename, ..., bylayer = FALSE, nogit = FALSE) {
  ...
  if(bylayer == TRUE & !nogit) {
    ...
    if(!nogit) git_prov(paste(filename, '.shp', sep = ''), filetype = 'output')
  }
}
...
plot_prov <- function(df) {
  ...
  df <- df %>%
    ...
    mutate(from = as.character(rdf_subject),
           to   = as.character(rdf_object),
           rel  = as.character(rdf_predicate)) %>%
    mutate(from = ifelse(filetype == 'output', as.character(rdf_object), from),
           to   = ifelse(filetype == 'output', as.character(rdf_subject), to),
           rel  = ifelse(filetype == 'output', 'generated', rel))

  nodes_df <- df %>%
    dplyr::select(file_loc, filetype, commit_url) %>%
    mutate(nodes = file_loc,
           label = basename(as.character(file_loc)),
