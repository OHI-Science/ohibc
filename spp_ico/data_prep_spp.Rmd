...
write_csv(spp_all_bcsee, file.path(dir_git, scenario, 'int/spp_all_bcsee.csv'))
...
knitr::kable(head(iucn_cells_sum),
             align = c('c', 'c', 'c', 'c', 'c', 'c'),
...
knitr::kable(head(summary_by_loiczid),
             caption = 'summary by LOICZID (head)')

knitr::kable(head(summary_by_rgn),
             caption = 'summary by region (head)')

```

## Summarize mean category/trend per cell/region, IUCN & BCSEE

This section performs similar calculations, but here calculates extinction risk score based on NatureServe province-level rankings where available, and IUCN category ranks elsewhere.  IUCN ranks exclude subpopulation scores due to lack of spatially explicit distribution info for subpopulations.

``` {r summing by cells and rgn 2, echo = FALSE, message = FALSE, warning = FALSE}
...
knitr::kable(head(sum_by_cell_bcsee),
             caption = 'summary by LOICZID (head)')

knitr::kable(head(sum_by_rgn_bcsee),
             caption = 'summary by region (head)')
...
write_csv(spp_status_bcsee, file.path(dir_git, scenario, 'data/spp_status_bcsee.csv'))
write_csv(spp_trend_bcsee,  file.path(dir_git, scenario, 'data/spp_trend_bcsee.csv'))
...
poly_plot_scores(spp_status_bcsee, scale_label = 'SPP Status', title = 'OHIBC SPP Status, IUCN + NS ranks')

knitr::kable(spp_trend %>%
  full_join(poly_bc_rgn@data, by = 'rgn_id'),
  align = c('c', 'c', 'l', 'c', 'r', 'r'),
  caption = 'SPP Trend by region:')

poly_plot_scores(spp_trend_bcsee, scale_label = 'SPP Trend', title = 'OHIBC SPP Trend', scale_limits = c(min(spp_trend$score), 0))
