...
``` {r read bcsee list, echo = TRUE, eval = TRUE, message = FALSE, warning = FALSE}
...
bcsee_syn_semicolon <- bcsee_all %>%
  ...
  separate(scisyn, c('syn1', 'syn2', 'syn3'), sep = ';', fill = 'right', remove = TRUE) %>%
...
bcsee_clean <- bcsee_no_syn %>%
  separate(sciname, c('sciname', 'var'), sep = ' var. ', fill = 'right', remove = TRUE) %>%
  separate(sciname, c('sciname', 'pop'), sep = ' pop. ', fill = 'right', remove = TRUE) %>%
  separate(sciname, c('sciname', 'ssp'), sep = ' ssp. ', fill = 'right', remove = TRUE) %>%
  select(-var, -pop, -ssp) %>%
  unique()

### Now the cleaned BCSEE data can be compared, via sciname, to spp_all.
nrow(bcsee_clean %>% filter(sciname %in% spp_all$sciname)) ### 327 species in common; but how many have spatial info?
spp_all1 <- spp_all %>%
  left_join(bcsee_clean, by = 'sciname') %>%
  filter(!is.na(spatial_source))
nrow(spp_all1 %>% filter(!is.na(status_gl) | !is.na(status_pr))) ### 248 BCSEE listed species remain...
...
spp_all1 <- spp_all1 %>%
  separate(status_pr, c('status_pr', 'status_pr2'), sep = ',', remove = TRUE, fill = 'right') %>%
  select(-status_pr2) %>%
...
spp_plot_raster(as.data.frame(summary_by_loiczid), loiczid_rast, 
                ...
                scale_limits = range(summary_by_loiczid$weighted_mean_cat))
...
cat(sprintf('Writing SPP status and trend for OHIBC assessment regions to:\n  %s\n  %s\n', 
