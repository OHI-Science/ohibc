...
From the region-level summaries of extinction risk and population trend, we calculate regional status and trend scores.  Currently, the Species Richness sub-goal model is identical to the OHI Global model: a region's status is based upon an area-weighted average of IUCN extinction risk.
...
While the tables show status and trend for both iterations (IUCN rankings only and IUCN + NatureServe rankings), these status and trend plots show only the original version (IUCN rankings only)
