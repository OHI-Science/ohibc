...
mdl <- lm(category_score ~ status_pr_score, data = spp_natureserve)
...
knitr::kable(spp_natureserve %>%
                select(sciname, spatial_source, iucn_cat = popn_category, 
                       IUCN_score = category_score, NSgl_score = status_gl_score, 
                       NSpr_score = status_pr_score, NSgl_rank = status_gl, NSpr_rank = status_pr) %>%
                arrange(sciname),
