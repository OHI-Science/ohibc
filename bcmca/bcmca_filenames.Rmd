...
get_layer <- function(x) {
  ifelse(x[1] == 'bcmca',
    str_replace(x[str_detect(x, '.shp')], '.shp', ''),
...
# Read .shps, rasterize, save?
Shape files are in BC Albers projections?  For Marxan layers, raster resolution is generally set to 2 km cells... let's start there... what is the origin used for the Marxan cells?

``` {r read .shps and rasterize}
...
#     message(sprintf('Layer %s is a points layer... figure out later!', shps_df$layer[i]))
    ...
    bcmca_poly <- readOGR(dsn = shp_dir, layer = shp_lyr)
    bcmca_p4s <- proj4string(bcmca_poly)
    
    if(bcmca_p4s != p4s_bcalb) {
      message(sprintf('Mismatched projections?\n  Layer p4s: %s\n  BC Albers p4s: %s', shps_df$layer[i], bcmca_p4s, p4s_bcalb))
      shps_df$p4s_match[i] = FALSE
    } else shps_df$p4s_match[i] = TRUE
    
    bcmca_ext <- c('xmin' = bcmca_poly@bbox[1, 1], 'xmax' = bcmca_poly@bbox[1, 2], 'ymin' = bcmca_poly@bbox[2, 1], 'ymax' = bcmca_poly@bbox[2, 2])
    bcmca_data <- names(bcmca_poly@data)
    
    shps_df$extent[i]    <- paste(bcmca_ext, collapse = ', ')
    shps_df$data_cols[i] <- paste(bcmca_data, collapse = ', ')
  }
}
...
# Remove duplicated data layers

Many of the basic shape files ("feature data" denoted by _data tag) were converted into Marxan shapefiles (denoted by _marxan tag).  Remove duplicated layers, keeping the original shapefiles rather than the Marxan files.

``` {r cull list to non dupes preferring non marxan, echo = FALSE, message = FALSE, warning = FALSE}
if(!exists('shps_df')) shps_df <- read.csv(file.path(dir_git_bcmca, 'bcmca_shps_all.csv'), stringsAsFactors = FALSE)

shps_df1 <- shps_df %>%
  filter(dupes == FALSE | (dupes == TRUE & !str_detect(tolower(shp), 'marxan')))

DT::datatable(shps_df1,
...
``` {r rasterize layers}
bcalb_shps <- shps_df1 %>%
  filter(p4s_match == TRUE) ### start with just layers already in BC Albers; later can spTransform the others...

for(i in 1:nrow(bcalb_shps)) { # i = 32
  tmp <- bcalb_shps[i, ]
  layername <- paste(tmp$category, tmp$layer, tmp$data_type, sep = '_')
  
  message(sprintf('Preparing layer %s...', layername))
  ...
  print(head(tmp_data))
  index_name <- names(tmp_poly)[max(c(1, which(str_detect(tolower(names(tmp_poly)), 'id'))[1]))]
#   tmp_rast <- raster::rasterize(tmp_poly, rast_base, field = 1, filename = file.path(dir_git_bcmca, sprintf('rasters/%s.tif', layername)))
  tmp_filename <- file.path(dir_anx_bcmca, sprintf('rasters/%s.tif', layername))
