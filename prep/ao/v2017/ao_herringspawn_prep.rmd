...
#   distinct()
# write_csv(shi_sec, file.path(dir_goal, 'raw/shi_sections_raw.csv'))
### ADD rgn_id AND rgn_wgt FIELDS MANUALLY and rename without _raw

shi_fix <- read_csv(file.path(dir_goal, 'raw/shi_secs_67_135_136.csv'),
...
### Alternate method with Herring Sections from DFO geodatabase

Note that this method is more "repeatable" than the previous, using area-weighting of a section across region boundaries to assign harvest values.  For the most part, the results appear nearly indistinguishable from the previous method, except for Aristazabal Island, which comes out looking better than the other version.  This is the method that will be used in the analysis.

``` {r extract_herring_section_by_rgn}

hsect_to_rgn_file <- file.path(dir_goal, 'herring_maps/hsect_to_rgn.csv')
...
shi_sec <- read_csv(hsect_to_rgn_file) %>%
  group_by(section) %>%
  mutate(rgn_wgt = area_km2 / sum(area_km2))

### This part reads the manual lookup table - commented out in favor of .gdb spatialization
# shi_sec <- read_csv(file.path(dir_goal, 'raw/shi_sections.csv')) %>%
#   select(-area_name) %>%
#   distinct()
...
Additionally, Karen Hunter of the DFO provided spawn data from 2005-2015 divided into PFMA subareas that may be useful to compare to the Spawn Habitat Index, though on its own, its utility is limited by the relatively short time series.
...
hspawn_dfo_raw <- lapply(sheets, FUN = function(x) {
  # x <- sheets[1]
    ...
    z <- y %>%
      ...
      filter(!is.na(year)) %>%
      gather(area, ssb, everything(), -year, -contains('total')) %>%
      mutate(dfo_rgn = x)
    return(z)
  }) %>%
  bind_rows() %>%
  mutate(area = str_replace(area, '-', '') %>% as.integer())

shi_sec <- read_csv(file.path(dir_goal, 'raw/shi_sections.csv')) %>%
  select(-area_name) %>%
  mutate(area = str_replace(area, 'Section ', '') %>% as.integer())

hspawn_dfo_clean <- hspawn_dfo_raw %>%
  left_join(shi_sec, by = 'area')
