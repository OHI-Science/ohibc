...
Possible reference point: mean herring spawn index from 1940-1960?  For each DFO larger region, let's plot this.  These are not OHIBC analysis regions.
...
shi_ohibc <- shi_df %>%
  filter(str_detect(tolower(area), 'section')) %>%
  select(area, area_name, year, tot_records, spawn_hab_index) %>%
  distinct() %>%
  left_join(shi_sec, by = c('area')) %>%
  group_by(rgn_id, year) %>%
  summarize(shi_tot = sum(spawn_hab_index * rgn_wgt, na.rm = TRUE)) %>%
  ungroup() %>%
  left_join(get_rgn_names(), by = 'rgn_id')

write_csv(shi_ohibc, file.path(dir_goal, 'int/shi_by_rgn.csv'))

```

``` {r plot_shi_vs_ref}

shi_by_rgn <- read_csv(file.path(dir_goal, 'int/shi_by_rgn.csv'))
...
shi_ohibc_05_15 <- read_csv(file.path(dir_goal, 'int/shi_by_rgn.csv')) %>%
