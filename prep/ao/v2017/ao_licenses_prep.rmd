...
Note, this does not catch First Nations members who are living separately from these communities, and assumes all members of the communities are First Nations members.
...
pop_cat_csd <- pop_csd %>%
  ...
  summarize(fn_pop = sum(pop * fn_pop),
            tot_pop = sum(pop),
            pct_fn = round(fn_pop / tot_pop, 5))


knitr::kable(pop_cat_csd, caption = 'First Nations communities as percent of total pop')

write_csv(pop_cat_csd, file.path(dir_goal, 'int/ao_licenses_fn_pop.csv'))
...
``` {r prov_footer, results = 'asis', eval = TRUE}
