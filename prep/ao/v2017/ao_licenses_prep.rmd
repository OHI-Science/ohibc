...
if(!file.exists(lic_clean_file)) {
  ...
  licenses_clean <- licenses_raw %>%
    ...
    mutate(year = licence_suffix,
           ...
           # fn = ifelse(fn == FALSE, str_detect(tolower(licence_holder_type), 'aborig'), fn)) %>%
              ### This assignment removed - don't assign FN based on ownership, but designation
    select(year, 
...
lic_clean <- read_csv(lic_clean_file, 
  ...
  mutate(year = as.integer(year),
         fn   = as.logical(fn),
         area = tolower(area),
         area = ifelse(is.na(area), 'bc coast wide', area))
