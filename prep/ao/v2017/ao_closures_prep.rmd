...
closure_areas <- read_csv(file.path(dir_goal, 'int/closure_dates_cleaned.csv')) %>%
  select(order_no, issued_date, closes_area) %>%
...
closure_areas_to <- closure_areas %>%
  ...
  group_by(order_no, issued_date, closes_area) %>%
...
closure_areas_clean <- closure_areas_easy %>%
  select(order_no, issued_date, closes_area, area, subarea) %>%
...
date_clip <- function(closures_df) {
  tmp <- closures_df %>%
    select(order_no, year, area, subarea, date_start, date_end, days) %>%
    distinct() %>%
...
closures <- closure_dates %>%
  inner_join(closure_areas, by = c('order_no', 'issued_date', 'closes_area')) %>%
  select(-species) %>%
...
write_csv(pfmsa_3nm_df, file.path(dir_goal, 'int/closure_pfmsa_to_rgn.csv'))
...
write_csv(closures_rgn_yr, file.path(dir_goal, 'int/closures_rgn_yr.csv'))

```

``` {r plot_closures_vs_time}

closures_rgn_yr <- read_csv(file.path(dir_goal, 'int/closures_rgn_yr.csv')) %>%
