...
* chemical pollution, which includes land-based organic chemicals (pesticides), land-based inorganic chemicals (impervious surface runoff), and ocean-based pollution from shipping and harbors.
    * processed by cw_chem_prep.Rmd
    * in the global analysis, this is assessed using the 3 nm coastal buffer zone.
    * details: 
        * years       : 2002 - 2013
...
* nutrient pollution, which includes nutrient runoff from agriculture
    * processed by cw_nutrient_prep.Rmd
    * in the global analysis, this is assessed at the 3 nm coastal buffer zone.
    * details: 
        * years       : 2002 - 2013
        * resolution  : 1000, 1000  (x, y) in m
        * coord. ref. : `+proj=aea +lat_1=50 +lat_2=58.5 +lat_0=45 +lon_0=-126 +x_0=1000000 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs` 
...
* marine debris, which includes marine plastics debris surface density by mass
    * processed by cw_trash_prep.Rmd
    * in the global analysis, this is assessed across the entire EEZ.
    * details: 
...
write_csv(nutr_score_df, file.path(dir_goal, 'output', 'cw_nutr_rgn_scores.csv'))

nutr_map <- tm_shape(nutr_stack[[nlayers(nutr_stack)]]) + 
...
write_csv(chem_score_df, file.path(dir_goal, 'output', 'cw_chem_rgn_scores.csv'))
...
if(length(trash_layer_files) > 1) 
  stop('More than one trash rescaled layer file! \n  ', paste(trash_layer_files, collapse = '\n  '))

trash_rast <- raster(trash_layer_files) %>%
  setNames(trash_layer_files %>% 
             basename %>% 
             str_replace('.tif', ''))
...
write_csv(trash_score_df, file.path(dir_goal, 'output', 'cw_trash_rgn_scores.csv'))

trash_map <- tm_shape(trash_rast) + 
  tm_raster(palette = 'Reds')
