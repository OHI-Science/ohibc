...
n_pac_ext <- extent(c(-180, -100, 0, 90))
...
writeRaster(vs_mass_npac, 
            file.path(dir_goal_anx, 'int/cw_trash_mass_vansebille_n_pac.tif'), 
            overwrite = TRUE)
...
trash_dens_interp_file <- file.path(dir_goal_anx, 'int/cw_trash_mass_interp.tif')
...
rast_dens_interp <- raster(file.path(dir_goal_anx, 'int/cw_trash_mass_interp.tif'))
...
trash_dens_lin <- raster(file.path(dir_goal_anx, 'int/cw_trash_mass_bcalb.tif'))

# print(hist(trash_dens_lin, main = 'Mass density (log(g/km2))'))

### bring in N Pacific raster; we are not mapping so scale and projection are unimportant
trash_dens_npac <- raster(file.path(dir_goal_anx, 'int/cw_trash_mass_vansebille_n_pac.tif'))
...
trash_ref_9999qtile_npac <- quantile(trash_dens_npac, prob = 0.9999)
...
trash_110max <- calc(trash_dens_lin, fun = function(x) {ifelse(x > trash_ref_110max, 1, x / trash_ref_110max)}) %>%
  setNames('linear_mass')
trash_9999qtile_npac <- calc(trash_dens_lin, fun = function(x) {ifelse(x > trash_ref_9999qtile_npac, 1, x / trash_ref_9999qtile_npac)}) %>%
  setNames('linear_mass')


writeRaster(trash_9999qtile, file.path(dir_goal_anx, 'rescaled_layers/cw_trash_9999qtile_eez_lin.tif'), overwrite = TRUE)
writeRaster(trash_110max, file.path(dir_goal_anx, 'rescaled_layers/cw_trash_110max_eez_lin.tif'), overwrite = TRUE)
writeRaster(trash_9999qtile_npac, file.path(dir_goal_anx, 'rescaled_layers/cw_trash_9999qtile_npac_lin.tif'), overwrite = TRUE)

plot(trash_9999qtile, main = 'CW trash pressure layer: \n mass density/99.99%ile in BC EEZ')
...
writeRaster(trash_log_9999qtile, file.path(dir_goal_anx, 'rescaled_layers/cw_trash_9999qtile_eez_log.tif'), overwrite = TRUE)
writeRaster(trash_log_110max, file.path(dir_goal_anx, 'rescaled_layers/cw_trash_110max_eez_log.tif'), overwrite = TRUE)
writeRaster(trash_log_9999qtile_npac, file.path(dir_goal_anx, 'rescaled_layers/cw_trash_9999qtile_npac_log.tif'), overwrite = TRUE)
...
* 99.99%ile within BC EEZ:  `r exp(trash_ref_9999qtile) - 1` g/km^2^ (log(x+1) = `r trash_logref_9999qtile`)
    * Linear-scaled file at: ``r file.path(dir_goal_anx, 'rescaled_layers/cw_trash_9999qtile_eez.tif')``
    * Log-scaled file at: ``r file.path(dir_goal_anx, 'rescaled_layers/cw_trash_9999qtile_eez_log.tif')``
* 110% of max within BC EEZ:  `r trash_ref_110max` (equiv to `r exp(trash_logref_110max) - 1` g/km^2^)
    * Linear-scaled file at: ``r file.path(dir_goal_anx, 'rescaled_layers/cw_trash_110max_eez.tif')``
    * Log-scaled file at: ``r file.path(dir_goal_anx, 'rescaled_layers/cw_trash_110max_eez_log.tif')``
* 99.99%ile within NE Pacific:  `r trash_ref_9999qtile_npac` (equiv to `r exp(trash_logref_9999qtile_npac) - 1` g/km^2^)
    * Linear-scaled file at: ``r file.path(dir_goal_anx, 'rescaled_layers/cw_trash_9999qtile_npac.tif')``
    * Log-scaled file at: ``r file.path(dir_goal_anx, 'rescaled_layers/cw_trash_9999qtile_npac_log.tif')``
