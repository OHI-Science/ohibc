...
if(any(!file.exists(new_rast_files))) { # i = 1
...
pest_stack <- stack(as.list(pest_layers_files))
...
pest_ref <- quantile(pest_log, prob = c(0.001, 0.01, 0.1, 0.25, 0.5, 0.75, 0.9, 0.99, 0.999, 0.9999))
  ### returns a quantile set for each layer
pest_99 <- pest_ref[ , 10] %>% max()
  ### column 10 is 99.99%; find max over all layers

pest_rescale <- calc(pest_log, fun = function(x) {
    ifelse(x > pest_99, 1, x / pest_99)
  }) %>%
...
op_newref <- quantile(op_bcalb, prob = c(0.001, 0.01, 0.1, 0.25, 0.5, 0.75, 0.9, 0.99, 0.999, 0.9999))
op_new99 <- op_newref[10]

op_bcalb_rescale <- op_bcalb / op_new99
values(op_bcalb_rescale)[values(op_bcalb_rescale) > 1] <- 1
...
ip_newref <- quantile(ip_bcalb, prob = c(0.001, 0.01, 0.1, 0.25, 0.5, 0.75, 0.9, 0.99, 0.999, 0.9999))
ip_new99 <- ip_newref[10]

ip_bcalb_rescale <- ip_bcalb / ip_new99
values(ip_bcalb_rescale)[values(ip_bcalb_rescale) > 1] <- 1
...
writeRaster(chem_stack_sum, 
            filename  = file.path(dir_goal_anx, 'rescaled_layers', paste0(names(chem_stack), '.tif')),
            bylayer   = TRUE,
            overwrite = TRUE)
...
for (i in 1:nlayers(pest_rescale)) { # i = 1
  year <- names(pest_rescale[[i]]) %>%
               str_replace('cw_pest', 'cw_geom_mean')
  message('Processing ', year)
  chem_year <- stack(list(ocean_rescale, inorg_rescale, pest_rescale[[i]]))
