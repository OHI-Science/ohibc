...
pop_cat_rgn_yr_plot <- ggplot(pop_cat_rgn_yr_sum %>%
  ...
  ylim(c(0, NA)) +
  theme(axis.text.x = element_text(angle = 30)) +
  labs(x = 'year', y = 'Population in size category', color = 'municipal\nsize category') +
  facet_wrap( ~ rgn_name, scales = 'free_y')
...
write_csv(ohibc_sum_sewer, file.path(dir_goal, 'output', 'po_pathogens.csv'))
...
ohibc_sum_sewer <- read_csv(file.path(dir_goal, 'output', 'po_pathogens.csv'))
...
prs_plot <- ggplot(prs_df, aes(x = year, y = path_pressure, color = rgn_name)) +
  ggtheme_plot() +
  geom_line(aes(group = rgn_name), size = 2, alpha = .8) +
  scale_color_brewer(palette = 'Dark2') +
