...
if(any(!file.exists(bc_stack_filenames)) | reload) {
  ...
  message('BC clipped annual average aragonite layers already exist')
...
if(!file.exists(arag_animation_file) | reload) {
  ...
  arag_stack <- stack(bc_stack_filenames) %>%
    projectRaster(ohibc_rgn_rast, method = 'ngb')
  
  names(arag_stack) <- paste0("OA_mean_arag_", str_extract(names(arag_stack), '[0-9]{4}'))
  
  animate_rast(arag_stack, 
               gif_file = arag_animation_file)
}

git_prov(arag_animation_file, filetype = 'plot')
...
oaRescale <- function(rast_file, reload = FALSE) {
  ...
  if(!file.exists(rescaled_rast_file) | reload) {
    ...
    values(rast_rescale)[!is.na(values(rast_rescale)) & 
                           values(rast_rescale) > 1]  <- values(rast_diff)[!is.na(values(rast_mean)) & 
                                                                             values(rast_mean) > 1] 
    ...
    message('File ', rescaled_rast_file, ' already exists; using existing layer')
    git_prov(rast_file,          filetype = 'input')
    git_prov(rescaled_rast_file, filetype = 'output')
  }
}
...
for(rast_file in arag_raw_files) oaRescale(rast_file, reload = FALSE)
...
for(rast_file in arag_resc_files) {
  ...
  output_file <- file.path(rast_file %>%
                             str_replace('/2_', '/3_') %>%
                             str_replace('rescaled', 'resc_interp'))
  
  if(!file.exists(output_file) | reload) {
    ...
    writeRaster(rast_interp, output_file,
                overwrite = TRUE)
  } else {
    message('File ', output_file, ' already exists; skipping it')
    git_prov(output_file, filetype = 'output')
  }
}
...
oaResample <- function(rast_file, reload = FALSE) {
  ...
  if(!file.exists(out_rast_file) | reload) {
    ...
    message('File ', out_rast_file, ' already exists; skipping')
    git_prov(rast_file,     filetype = 'input')
    git_prov(out_rast_file, filetype = 'output')
  }
}

arag_resc_interp_files <- list.files(file.path(dir_goal_anx, 'oa_rasts'), pattern = 'resc_interp', full.names = TRUE)

for(rast_file in arag_resc_interp_files) oaResample(rast_file, reload = FALSE)
...
oa_prs_animation_file <- file.path(dir_goal, 'Figs', 'oa_pressure_animation.gif')
reload <- FALSE
...
git_prov(oa_prs_animation_file, filetype = 'plot')
...
plot(x2012, main = 'Diff between global and local calcs 2012')

hist(x2012, 
     main = 'Cell difference % (local - global / local) 2012',
     xlab = 'percent difference', ylab = 'number of cells')

x2015 <- (loc_2015 - gl_2015) / loc_2015

plot(x2015, main = 'Diff between global and local calcs 2015')

hist(x2015, 
     main = 'Cell difference % (local - global / local) 2015',
     xlab = 'percent difference', ylab = 'number of cells')
...
writeRaster(rast_gapfill_masked,
            filename = file.path(dir_goal, 'oa_rasts', '6_prs_oa_bc_gapfill.tif'),
            overwrite = TRUE)
```
...
if(!file.exists(gf_map_file) | reload) {
  ...
  gf_rast <- raster(file.path(dir_goal, 'oa_rasts', '6_prs_oa_bc_gapfill.tif')) %>%
  ...
  gf_plot <- tm_shape(gf_rast) + 
    ...
    tm_layout(title = 'Gapfills: +1 = data, -1 = gap')
