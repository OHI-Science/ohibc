...
topics <- c('income', 'labour force', 'population') %>%
  paste(collapse = '|')
census_010611 <- bind_rows(census_2001, census_2006, census_2011) %>%
  filter(str_detect(tolower(topic), topics))

# y <- census_010611 %>%
#   select(year, characteristic) %>%
#   distinct()

characteristics <- c('Median total income', 'Median household total', 
                     'Median income', 'Employment rate', 'Unemployment rate') %>%
  paste(collapse = '|')

income_employment_010611 <- census_010611 %>%
  filter(str_detect(characteristic, characteristics))
...
income_2016_file <- file.path(dir_goal, 'int/income_only_2016.csv')
census_2016_raw_file <- file.path(dir_goal, 'int/census_raw_2016.csv')
### 2016 data are saved as a SDMX file for entire Canada
census_2016_gen_file <- file.path(dir_data_anx, 'canada_census',
                               '98-400-X2016099/Generic_98-400-X2016099.xml')
census_2016_str_file <- file.path(dir_data_anx, 'canada_census',
                               '98-400-X2016099/Structure_98-400-X2016099.xml')

if(!file.exists(census_2016_raw_file)) {
  
  # devtools::install_github("opensdmx/rsdmx")
  census_2016_gen <- rsdmx::readSDMX(census_2016_gen_file, isURL = FALSE) %>%
    as.data.frame() %>%
    clean_df_names()
  census_2016_str <- rsdmx::readSDMX(census_2016_str_file, isURL = FALSE)
  
  ### get codelists from structure
  cls <- slot(census_2016_str, "codelists")
  ...
  ### get codelist
  geo_codes <- as.data.frame(slot(census_2016_str, "codelists"), 
                                codelistId = 'CL_GEO') %>%
    setNames(c('geo', 'csd_name'))
  char_codes <- as.data.frame(slot(census_2016_str, "codelists"), 
                                         codelistId = 'CL_A16_HHTYPE_CFSTRUCT') %>%
    setNames(c('a16_hhtype_cfstruct', 'value_fr', 'characteristic')) %>%
    select(-value_fr)
  
  income_codes <- as.data.frame(slot(census_2016_str, "codelists"), codelistId = 'CL_HHINC_GRP') %>%
    setNames(c('hhinc_grp', 'value_fr', 'income_type')) %>%
    select(-value_fr)
  
  codes4 <- as.data.frame(slot(census_2016_str, "codelists"), codelistId = codelists[4]) %>%
    setNames(c(codelists[4], 'value_fr', 'value')) %>%
    select(-value_fr) %>%
    clean_df_names()
  ...
  write_csv(census_2016, census_2016_raw_file)
} else {
  git_prov(c(census_2016_gen_file, census_2016_str_file), filetype = 'input')
  git_prov(census_2016_raw_file, filetype = 'output')
}

income_2016 <- read_csv(census_2016_raw_file) %>%
  filter(str_detect(income_type, 'Median total income')) %>%
  filter(str_detect(characteristic, 'Total - Household type'))

write_csv(income_2016, income_2016_file)
...
### combine data with regions and gapfill NAs
unemp_df <- read_csv(file.path(dir_goal, 'int', 'inc_and_empl_yr_csd.csv')) %>%
...
unemp_wt_mean <- unemp_df %>%
  group_by(year, rgn_id) %>%
  mutate(dist_unemp = as.numeric(value),
         rgn_mean_unemp = sum(value * ohibc_pop, na.rm = TRUE) / sum(ohibc_pop),
         rgn_mean_unemp = round(rgn_mean_unemp, 2)) %>%
  group_by(year, rgn_id, fn, rgn_mean_unemp) %>%
  summarize(mean_unemp = sum(value * ohibc_pop, na.rm = TRUE) / sum(ohibc_pop),
            mean_unemp = round(mean_unemp, 2),
            category_pop = sum(ohibc_pop, na.rm = TRUE)) %>%
  ungroup() %>%
  filter(mean_unemp > 0)

unempl_wt_mean_allyrs <- unemp_wt_mean %>%
  group_by(rgn_id, fn) %>%
  complete(year = min(year):max(year), nesting(rgn_id)) %>%
  mutate(rgn_mean_unemp = zoo::na.approx(rgn_mean_unemp, year),
         mean_unemp     = zoo::na.approx(mean_unemp, year),
         category_pop   = zoo::na.approx(category_pop, year)) %>%
  ungroup()

fn_unempl <- unempl_wt_med_allyrs %>%
  filter(fn) %>%
  select(year, rgn_id, median_unempl)

non_fn_unempl <- unempl_wt_med_allyrs %>%
  filter(!fn) %>%
  select(year, rgn_id, median_unempl)

fn_pop_unempl <- unempl_wt_med_allyrs %>%
  filter(fn) %>%
  select(year, rgn_id, fn_pop = category_pop)

non_fn_pop_unempl <- unempl_wt_med_allyrs %>%
  filter(!fn) %>%
  select(year, rgn_id, non_fn_pop = category_pop)
...
DT::datatable(unemp_wt_mean_allyrs, caption = 'Mean unemployment')
...
ggplot(unempl_layer, aes(x = year, y = mean_unemp, color = fn)) +
  ggtheme_plot() +
  geom_line(aes(group = fn), alpha = .7, size = 1.5) +
  scale_x_continuous(breaks = c(1996:2016)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
