...
cens_file_2016 <- census_files[str_detect(census_files, '2016')]
...
# "bcprep/_raw_data/le/census_data/98-400-X2016287_English_CSV_data.csv"
### This is by Census Metropolitan Area and Census Agglomeration
# "bcprep/_raw_data/le/census_data/98-402-X2016006-T1-CSD-Eng.csv"
### This is by census subdivision looks like.
### For simplicity, get all at same scale and use one shapefile...
census_2016 <- read_csv(cens_file_2016[2]) %>%
  clean_df_names() %>%
  filter(str_detect(alt_geo_code, '^59'))
...
``` {r map le scores, fig.height = 6, eval = FALSE}

rgns <- read_sf(dir_spatial, 'ohibc_rgn_simple') %>%
  mutate(rgn_id = as.integer(rgn_id))

continent <- read_sf(dir_spatial, 'ohibc_continent')

scores_from_toolbox <- read_csv(file.path(dir_git, 'calc_ohibc/scores_all.csv')) %>%
  filter(goal == 'LE' & dimension == 'status') %>%
  filter(year == max(year))

rgn_scores <- rgns %>%
  left_join(scores_from_toolbox, by = c('rgn_id' = 'region_id')) %>%
  mutate(score = round(score))

ggplot(rgn_scores) +
  ggtheme_plot() +
  geom_sf(data = continent, fill = 'grey90', color = 'grey50', size = .25, alpha = .7) +
  geom_sf(aes(fill = score), size = .25, color = 'grey40', alpha = .7) +
  scale_fill_distiller(palette = 'Spectral', direction = 1, limits = c(0, 100))

library(tmap)
tm_shape(continent) +
  tm_polygons(col = 'grey80', border.col = 'grey50', size = .25) +
tm_shape(rgn_scores, is.master = TRUE) +
  tm_polygons(col = 'score', border.col = 'grey40', 
              style = 'cont', 
              palette = 'Spectral', 
              legend.show = FALSE,
              colorNA = 'grey90') +
tm_shape(rgn_scores %>% filter(rgn_id != 7)) +
  tm_bubbles(size = 3, col = 'white', alpha = .8) +
  tm_markers(text = 'score', text.just = c('center', 'center')) +
tm_shape(continent %>% filter(rgn_id == 218) %>% mutate(label = '93')) +
  tm_bubbles(size = 10, col = 'white', alpha = .8) +
  tm_markers(text = 'label', size.max = 3, size = 'label',
             text.just = c('center', 'center')) +
tm_layout(bg.color = '#aabbff')
