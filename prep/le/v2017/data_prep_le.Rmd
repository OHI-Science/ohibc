...
income_wt_median <- income_df %>%
  ...
  mutate(rgn_median_all = sum(value * ohibc_pop, na.rm = TRUE) / sum(ohibc_pop),
         rgn_median_all = round(rgn_median_all, 2)) %>%
  group_by(year, rgn_id, fn, rgn_median_all) %>%
  summarize(cat_median_income = sum(value * ohibc_pop, na.rm = TRUE) / sum(ohibc_pop),
            cat_median_income = round(cat_median_income, 2),
            category_pop  = sum(ohibc_pop, na.rm = TRUE)) %>%
  ungroup() %>%
  filter(cat_median_income > 0)

income_wt_med_allyrs <- income_wt_median %>%
  group_by(rgn_id, fn) %>%
  complete(year = min(year):max(year), nesting(rgn_id)) %>%
  mutate(rgn_median_all    = zoo::na.approx(rgn_median_all, year),
         cat_median_income = zoo::na.approx(cat_median_income, year),
         category_pop      = zoo::na.approx(category_pop, year)) %>%
  ungroup()

fn_income <- income_wt_med_allyrs %>%
  filter(fn) %>%
  select(year, rgn_id, fn_median_income = cat_median_income)

non_fn_income <- income_wt_med_allyrs %>%
  filter(!fn) %>%
  select(year, rgn_id, non_fn_median_income = cat_median_income)
...
DT::datatable(income_wt_med_allyrs %>% rename(first_nations = fn), caption = 'Median income')
...
unempl_wt_mean <- unempl_df %>%
  group_by(year, rgn_id) %>%
  mutate(rgn_mean_unempl_all = sum(value * ohibc_pop, na.rm = TRUE) / sum(ohibc_pop),
         rgn_mean_unempl_all = round(rgn_mean_unempl_all, 2)) %>%
  group_by(year, rgn_id, fn, rgn_mean_unempl_all) %>%
  summarize(cat_mean_unempl  = sum(value * ohibc_pop, na.rm = TRUE) / sum(ohibc_pop),
            cat_mean_unempl  = round(cat_mean_unempl, 2),
            category_pop = sum(ohibc_pop, na.rm = TRUE)) %>%
  ungroup() %>%
  filter(cat_mean_unempl > 0)
...
fn_unempl <- unempl_wt_mean_allyrs %>%
  filter(fn) %>%
  select(year, rgn_id, fn_mean_unempl = cat_mean_unempl)

non_fn_unempl <- unempl_wt_mean_allyrs %>%
  filter(!fn) %>%
  select(year, rgn_id, non_fn_mean_unempl = cat_mean_unempl)
...
``` {r data_viz, eval = TRUE}

income_layer <- read_csv(file.path(dir_goal, 'output', 'le_income_fn.csv')) %>%
  left_join(read_csv(file.path(dir_goal, 'output', 'le_income_nonfn.csv')),
            by = c('rgn_id', 'year')) %>%
  left_join(get_rgn_names(), by = 'rgn_id') %>%
  gather(first_nations, median_income, contains('income')) %>%
  mutate(first_nations = !str_detect(first_nations, 'non_fn'))

unempl_layer <- read_csv(file.path(dir_goal, 'output', 'le_unempl_fn.csv')) %>%
  left_join(read_csv(file.path(dir_goal, 'output', 'le_unempl_nonfn.csv')),
            by = c('rgn_id', 'year')) %>%
  left_join(get_rgn_names(), by = 'rgn_id') %>%
  gather(first_nations, mean_unempl, contains('unempl')) %>%
  mutate(first_nations = !str_detect(first_nations, 'non_fn'))

ggplot(income_layer, aes(x = year, y = median_income, color = first_nations)) +
  ggtheme_plot() +
