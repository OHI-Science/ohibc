...
Normalizing by the mean we recenter and rescale the distribution around the ideal value of 1.0.
...
E_dist <- stocks_rescaled %>%
  ...
  summarize(mean_E_Et    = mean(E_Et),
            sd_E_Et      = sd(E_Et),
            sd_norm_E_Et = sd_E_Et / mean_E_Et)
...
Using an approximate normalized standard deviation of $\sigma_E = 0.60$ and a max overescapement penalty of $\delta_E = .25$, we set thresholds away from the target (i.e. 1.0) at 1 std dev below, 1 std dev above, and 2 std devs above.

\begin{align*}
  E' &= \begin{cases}
    0                            &\text{ when } \frac{E}{E_t} < (1 - \sigma_E)\\
    E_{0,1}' + m_{E1} \frac{E}{E_t} &\text{ when } (1 - \sigma_E) \leq \frac{E}{E_t} < 1.0\\
    1.0                          &\text{ when } 1.0 \leq \frac{E}{E_t} < (1 + \sigma_E)\\
    E_{0,2}' + m_{E2} \frac{E}{E_t} &\text{ when } (1 + \sigma_E) \leq \frac{E}{E_t} < (1 + 2 \sigma_E)\\
    \delta_E                       &\text{ when } \frac{E}{E_t} \geq (1 + 2 \sigma_E)
  \end{cases}\\
  \text{Where: }\\
    m_{E1}      &= \frac{1}{\sigma_E}        = \frac{1}{.6}         =  1.67 \text{ for } \sigma_E = 0.60\\
    E_{0,1}' &= 1 - m_{E1}                   = \frac{-.4}{.6}       =  -.67 \text{ for } \sigma_E = 0.60\\
    m_{E2}      &= -\frac{1 - \delta_E}{\sigma_E} = -\frac{.75}{.6} = -1.25 \text{ for } \sigma_E = 0.60\\
    E_{0,2}' &= 1 - m_{E2} (1 + \sigma_E)    = 1 - (-1.25)(1.6)     =  3.00 \text{ for } \sigma_E = 0.60\\
\end{align*}
          
``` {r plot escapement control rule}
ecr_df <- data.frame(x = c(0, .4, 1, 1.6, 2.2, 3.0),
                     y = c(0,  0, 1,   1, .25, .25))

ggplot(ecr_df, aes(x, y)) +
  ggtheme_plot() +
  geom_line() +
  labs(x = 'E/E(target)',
       y = "Rescaled E' score")
...
Using an approximate normalized standard deviation of 1.20, we set an upper buffer at 1 std dev above the target.  Below $C/C_t = 1.0$ we set a buffer $\gamma_C = .40$, representing an allowance for uncertainty as well as intentional underharvest for ecological purposes (e.g. "one third for the birds" - and orcas, and bears, etc).  Below this threshold, $C'$ decreases to a minimum value of $\delta_C = 0.25$ at a catch (and thus $C/C_{target}$) of zero.

\begin{align*}
  C' &= \begin{cases}
    C_{0,1}' + m_{C1} \frac{C}{C_t}  &\text{ when } \frac{C}{C_t} < 1 - \gamma_C\\
    1                                &\text{ when } 1 - \gamma_C \leq \frac{C}{C_t} < 1\\
    C_{0,2}' + m_{C2} \frac{C}{C_t}  &\text{ when } 1 \leq \frac{C}{C_t} < 1 + \sigma_C\\
    0                                &\text{ when } \frac{C}{C_t} \geq 1 + \sigma_C
  \end{cases}\\
  \text{Where: }\\
    m_{C1}   &= \frac{1 - \delta_C}{1 - \gamma_C}  = \frac{.75}{.6} =  1.25\\
    C_{0,1}' &= \delta_C                                            =  0.25\\
    m_{C2}   &= -\frac{1}{\sigma_C}                = -\frac{1}{1.2} = -0.83 \text{ for } \sigma_C = 1.20\\
    C_{0,2}' &= -m_{C2} (1 + \sigma_C)             = +.83 (2.2)     =  1.83 \text{ for } \sigma_C = 1.20\\
\end{align*}
          
``` {r plot catch control rule}
ccr_df <- data.frame(x = c( 0.0, 0.6, 1.0, 2.2),
                     y = c(0.25, 1.0, 1.0, 0.0))

ggplot(ccr_df, aes(x, y)) +
  ggtheme_plot() +
  geom_line() +
  labs(x = 'C/C(target)',
       y = "Rescaled C' score")
...
calc_c_prime <- function(C_Ct) {
  ...
  sigma_c <- 1.2 ### std deviation of C/Ctarget
  
  m_c1 <- (1 - delta_c) / (1 - gamma_c)
  m_c2 <- - 1 / sigma_c
  c_0_1 <- delta_c
  c_0_2 <- -m_c2 * (1 + sigma_c)
  
  c_prime <- case_when(C_Ct < 1.0 - gamma_c  ~ c_0_1 + m_c1 * C_Ct,
                       C_Ct < 1.0            ~ 1.0,
                       C_Ct < 1.0 + sigma_c  ~ c_0_2 + m_c2 * C_Ct,
                       C_Ct >= 1.0 + sigma_c ~ 0,
                       TRUE                  ~ NA_real_)
}
```
...
``` {r provenance, results = 'asis', eval = FALSE}
