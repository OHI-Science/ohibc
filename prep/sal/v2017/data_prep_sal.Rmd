...
stocks_scored <- stocks %>%
  rowwise() %>%
  mutate(esc_score = min(escapes / esc_tgt, 1), ### NAs should also dominate the max
         catch_score = min(catch_tgt / catch, 1),
         stock_score = mean(c(esc_score, catch_score), na.rm = TRUE)) %>%
  ungroup() %>%
...
DT::datatable(stocks_scored)

ggplot(stocks_scored %>%
         filter(stock_spp != 'sockeye'), 
       aes(x = year, y = stock_score, color = stock)) +
  ggtheme_plot() +
  geom_line(size = 2) +
  geom_point(size = 3) +
  ylim(c(0, 1)) + 
  facet_wrap( ~ stock_spp)
