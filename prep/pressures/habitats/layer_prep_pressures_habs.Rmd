...
etopo_bc_file <- file.path(dir_rast, 'rast_bath_raw.tif')
if(!file.exists(etopo_bc_file)) {
  ...
  values(etopo_bcalb)[values(etopo_bcalb) > 0] <- NA
  
  writeRaster(etopo_bcalb, etopo_bc_file, overwrite = TRUE)
  
} else {
  
  etopo_bcalb <- raster(etopo_bc_file)
