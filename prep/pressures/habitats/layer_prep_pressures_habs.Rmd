...
rast_benthic_file <- file.path(dir_rast, 'rast_benthic.tif')

if(!file.exists(rast_benthic_file)) {
  message('Creating benthic raster')
  
  library(gdalUtils)
  ### set tmp directory for rasterizing
  tmpdir = file.path(dir_neptune_data, 'home_big/ohara/R_raster_tmp')
  dir.create(tmpdir, showWarnings=F)
  rasterOptions(tmpdir = tmpdir)
  
  ### figure out source file location for polygon
  zone_lyrs <- hab_lyrs %>% filter(category == 'zone')
...
#   rast_benthic <- gdal_rasterize(src_datasource = zone_shp, 
#                                  dst_filename   = rast_benthic_file,
...
#                                  output_Raster = TRUE)
  ...
  message(sprintf('Loading benthic raster from %s', rast_benthic_file)
  rast_benthic <- raster(rast_benthic_file)
}
...
rast_depthzone <- floor(rast_benthic/1000)
...
# poly_ohibc_rgn  <- readShapePoly(file.path(dir_rgn, 'ohibc_rgn'),  proj4string = CRS(p4s_bcalb))
# 
