...
``` {r rasterise_benthic_class, echo = FALSE}
...
``` {r create_zones_from_polys, echo = FALSE}
...
if(!file.exists(rast_zone_intertidal_file) | !file.exists(rast_zone_deep_file)) {
  message('Missing intertidal and/or deep zone raster(s)')
  ### intertidal is based off a land buffer; deep is related to Pacific Offshore region
  poly_intertidal <- readShapePoly(file.path(dir_rgn, 'ohibc_offshore_3nm'),
  ...
  message(system.time({
    rast_zone_intertidal <- rasterize(poly_intertidal, rast_base)
  }))
  ...
  message(sprintf('Intertidal and deep zone rasters exist; reading from %s and %s', 
          rast_zone_intertidal_file, rast_zone_deep_file))
