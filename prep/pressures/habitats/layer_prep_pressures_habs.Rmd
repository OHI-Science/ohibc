...
etopo_bc_file <- file.path(dir_rast, 'rast_etopo.tif')
if(!file.exists(etopo_bc_file)) {
  ...
  crs(etopo_rast1 <- "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0" 
  ...
  plot_raster(etopo_bcalb, scale_limits = c(min(values(etopo_bc_alb), na.rm = TRUE), 0))
  
  writeRaster(etopo_bcalb, etopo_bc_file)
}
