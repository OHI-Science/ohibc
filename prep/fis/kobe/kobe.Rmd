...
* To calculate B', we define categories in which a given stock is overfished, underfished, or adequately fished.
    * $B' = \frac{B/B_{MSY}}{0.8}$  when $B/B_{MSY}$ indicates an overfished stock ($B/B_{MSY} < 0.8$, where 0.8 is the overfished threshold)
    * $B' = 1$ when $B/B_{MSY}$ indicates an appropriately fished stock ($0.8 <= B/B_{MSY} < 1.5$, where 1.5 is the underfished threshold)
    * $B' = \frac{(B/B_{MSY})_{max} - (B/B_{MSY})}{(B/B_{MSY})_{max} - 1.5}$ when $B/B_{MSY}$ indicates underfished stock ($B/B_{MSY} >= 1.5$).  Note that $(B/B_{MSY})_{max}$ is the highest ratio found within the assessment region; here we will consider all fisheries within the entire BC EEZ)
* To calculate F', we examine fishing pressure within two regimes: overfished stock ($B/B_{MSY} < 0.8$) and not overfished stock.
    * $F' = 0$  when overfished, and grossly overfishing ($F/F_{MSY} > B/B_{MSY} + 1.5$)
    * $F' = \frac{(B/B_{MSY} + 1.5) - F/F_{MSY}}{1.5}$  when overfished, and moderately overfishing ($B/B_{MSY} + 0.2 < F/F_{MSY} <= B/B_{MSY} + 1.5$)
    * $F' = 1$  when overfished, and appropriately fishing ($B/B_{MSY} - 0.2 <= F/F_{MSY} < B/B_{MSY} + 0.2$)
    * $F' = \frac{F/F_{MSY}}{B/B_{MSY} - 0.2}$  when overfished, and underfishing ($F/F_{MSY} < B/B_{MSY} - 0.2$)

    * $F' = \frac{F/F_{MSY}}{0.8}$  when _not_ overfished, and underfishing ($F/F_{MSY} < 0.8$, where 0.8 is the underfishing threshold)
    * $F' = 1$  when _not_ overfished, and appropriate fishing ($0.8 <= F/F_{MSY} < 1.2$, where 0.8 and 1.2 are under- and over-fishing thresholds)
    * $F' = \frac{(F/F_{MSY})_{max} - F/F_{MSY}}{(F/F_{MSY})_{max} - 1.2}$  when _not_ overfished, and overfishing ($0.8 <= F/F_{MSY} < 1.2$, where 1.2 is the overfishing threshold)
...
The following plots assume a local maximum B/Bmsy of 3.5, and a local maximum F/Fmsy of 2.5.
