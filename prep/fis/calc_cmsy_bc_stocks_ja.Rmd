...
ca_stocks <- read_csv(file.path(dir_M,'git-annex/globalprep/fis/v2017/int/stock_catch_by_rgn.csv')) %>%
  filter(fao_rgn == 67,
         rgn_id==218,
         taxon_key > 599999) %>%
  group_by(taxon_scientific_name, taxon_common_name) %>%
  summarize(years = n(),
            avg_catch = mean(tons)) %>%
  mutate(run_cmsy = ifelse(years >=20 & avg_catch >=1000, "yes", "no"))
...
There are **81 stocks** in Canadian BC waters. Let's filter out the catch to just incude those found in Canadian waters. Also need to filter out stocks that dont have at elast 20 yr time series and an annual avg catch of 1000 tons
