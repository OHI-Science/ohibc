...
Rerun catch-MSY on all SAUP data for stocks in FAO areae 67 (British Columbia)
...
ca_stocks <- read_csv(file.path(dir_M,'git-annex/globalprep/fis/v2017/int/stock_catch_by_rgn.csv')) %>%
  filter(fao_rgn == 67,
         rgn_id==218,
         taxon_key > 599999) %>%
  select(sciname = taxon_scientific_name, common = taxon_common_name) %>%
  distinct()

nrow(ca_stocks) #81 stocks in CA waters. Let's filter out the catch to just incude those found in Canadian waters. Also need to filter out stocks that dont have at elast 20 yr time series and an annual avg catch of 1000 tons

catch_for_cmsy <- catch %>%
  filter(sciname %in% ca_stocks$sciname) %>%
  group_by(stock_id)%>%
  ...
  filter(nyrs > 19,
         avg_ann_catch >= 1000,
         str_detect(common, "almon")==FALSE)  # also removing salmon

length(unique(catch_for_cmsy$stock_id)) #there are only 27 stocks to run through CMSY
