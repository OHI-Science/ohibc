...
```{r}
halibut <- read_sf(file.path(dir_anx, '_raw_data/dfo_khunter/fisheries', 
                             'd2018/halibut/Halibut_2015.shp')) %>% 
  as.data.frame() %>%
  select(-geometry)
...
```{r}
shrimp <- read_sf(file.path(dir_anx, '_raw_data/dfo_khunter/fisheries',
                            'd2018/shrimp/Shrimp_2009.shp')) %>%
  as.data.frame() %>%
  select(-geometry)
...
read_files <- function(folder, column) {
  ...
  files <- list.files(file.path(dir_anx, '_raw_data/dfo_khunter/fisheries/d2018', folder), 
                      full.names = TRUE)
...
<!-- We are still waiting on two more years of data for Albacore tuna which is why I have it commented out. Otherwise this would throw an error since not all years are there.  -->
...
files <- list.files(file.path(dir_anx, '_raw_data/dfo_khunter/fisheries/d2018/shrimp'), 
                    full.names = TRUE)
  ...
  for(yr in 2007:2015){
    ...
    yr_shp <- read_sf(yr_shp_file) %>%
      as.data.frame() %>%
      mutate(harvest = Sum_pinks + Sum_sdstrp + Sum_prawns + Sum_hmpbck + 
               Sum_docks + Sum_smth_p + Sum_flxd_p + Sum_othr_w + Sum_spny_p) %>%
...
stock_rgn_prop <- readr::read_csv('ram/2_ram_stock_to_ohibc_areas.csv') %>%
  ...
  mutate(stock_area = sum(area_km2),   ### we want the total area of the 
           ### species within each BC region and then we calculate
         stock_rgn_prop = area_km2/stock_area) ### the proportion of each 
           ### BC region that contains the stock. This ensures that the 
           ### proportional area the two (or three) stocks in a region will add up to one
...
4. adding in stockids to link to assessment info  
    a. note that Strait of Georgia hake is unassessed, so this is manually set to NA stock ID
...
species_catch <- af %>%
  ...
  mutate(harvest_kgs = case_when(units == 'lbs'         ~ harvest/0.453592,  ### pounds to kg
                                 units == 'metric_tons' ~ harvest*1000, 
                                 TRUE                   ~ harvest)) %>%
  select(-units, -harvest) %>%
  left_join(stock_rgn_prop, by = c('species', 'rgn_name', 'rgn_id')) %>% 
    ### this adds in the calculated prop for multiple stocks but we need to 
    ### add in the stockids for the other stocks
  mutate(stockid = case_when(
           species == 'sablefish'     ~ 'SABLEFPCAN',
           species == 'bocaccio'      ~ 'BOCACCBCW',
           species == 'albacore_tuna' ~ 'ALBANPAC',
           species == 'hake' & rgn_id != 6 ~ 'PHAKEPCOAST',
             ### rgn_id 6 is Strait of Georgia - hake is unassessed
           species == 'halibut'       ~ 'PHALNPAC',
           species %in% c('pacific_cod', 'pacific_ocean_perch', 'lingcod', 'srsole') ~ stockid,
           TRUE ~ NA_character_),
         catch = ifelse(!is.na(stock_rgn_prop), harvest_kgs * stock_rgn_prop, harvest_kgs)) %>% 
           ### calculate the proportion of each stock harvested in each region. 
           ### This is only different for species with multiple stocks in BC
  mutate(assessed = ifelse(is.na(stockid), 0, 1)) %>%  
    ### identify stocks that are assessed
  group_by(rgn_name, rgn_id, rgn_code, year, assessed) %>%
  mutate(rgn_catch  = sum(catch)) %>% 
    ### this calculates the total catch (in kgs) for each region of either 
    ### assessed or unassessed species
  ungroup() %>%
  mutate(rgn_ass_catch_prop = case_when(is.na(stockid) ~ NA_real_, 
                                        rgn_catch == 0 ~ 0,  ### avoid NaNs
                                        TRUE           ~ catch/rgn_catch)) %>% 
    ### catch_prop gives the proportion of catch that harvest contributes 
    ### towards the overall assessed or unassessed catch. We are really only 
    ### going to use the assessed part of this.
  select(rgn_name, rgn_id, year, species, stockid, catch, rgn_catch, rgn_ass_catch_prop)
  
### Here we manually adjust the stockid to account for alt-assessed stocks:
alt_assess <- c('herring', 'geoduck', 'spot_prawn', 'shrimp', 'dungeness_crab')

species_catch <- species_catch %>%
  mutate(stockid = ifelse(is.na(stockid) & species %in% alt_assess,
                          'alternative assessment method, excluded from scoring',
                          stockid))

write_csv(species_catch, file.path(dir_goal, 'output/dfo_catch.csv'))
```
...
Update:  Based on conversations with Karen Hunter (DFO), there are assessments for invertebrates (shrimp, spot prawn, dungeness crab, and geoduck) as well as herring stocks, but not in a way that is readily usable for our F' and B' method.  As such, we will not count these stocks as "unassessed" for the purposes of the unassessed penalty.  To identify these stocks, see chunk above: we code the `stockid` variable as "alternative assessment method, excluded from scoring".

```{r}
rgn_catch <- species_catch %>%
  filter(is.na(stockid) | !str_detect(stockid, 'excluded')) %>%
    ### keep NA stockid (unassessed) and non-excluded (assessed)
  group_by(year, rgn_id, rgn_name) %>%
  summarize(rgn_catch = sum(catch))

unassessed <- species_catch %>%
  filter(is.na(stockid)) %>%  
    ### if a stockid is na it is not assessed.
    ### this will also drop the 'alternative assessment method, excluded from scoring' stocks
  group_by(rgn_name, rgn_id, year) %>%
  summarize(unass_catch = sum(catch)) %>%
  ungroup()

catch_summary <- rgn_catch %>%
  left_join(unassessed, by = c('year', 'rgn_id', 'rgn_name')) %>%
  mutate(unass_catch_prop = ifelse(rgn_catch == 0, 1, unass_catch/rgn_catch),
          ass_catch_prop  = 1 - unass_catch_prop)

write_csv(catch_summary, file.path(dir_goal, 'output/rgn_catch_summary.csv'))
```
