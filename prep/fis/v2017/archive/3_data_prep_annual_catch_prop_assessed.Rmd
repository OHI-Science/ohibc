...
assessed_spp <- read_csv("output/ram_b_bmsy.csv") %>%
  ...
  distinct() %>%
  filter(year > 1997) %>%    
    ### we only have DFO catch data from 1998 onwards so no need to include 
    ### the entire ram series here
  mutate(
    species = case_when(
...
for(i in 1:13){
  ...
  df <- sheet %>%
    ...
    filter(!is.na(species),
           tolower(variable) %in% c("landings", "harvest")) 
  ### values are in thousands of tons. since we're getting a prop we don't 
  ### need to multiply by 1000 here. Different years have landings or harvest as column names
...
yrly_catch <- landings_df %>%
  mutate(keep = ifelse(species %in% spp_total_groups, 1, 0)) %>%
    ### formerly, a check on 1998 to add roe, spawn, and bait for herring; but
    ### I added a herring row to the .xlsx which for some reason had been excluded.
    ### Taking out this check made sure that the result was identical to the original
    ### coded methods -- Casey
  filter(keep == 1) %>%
  group_by(year) %>%
...
### now calculate total landings for just the assessed catch. We want to 
### assume that any stocks with assessments in 2010 are also assessed through 
### 2015. Even though this isn't reported, there is likely a lag in 
### assessment time. We also need to remove all salmon and farmed sp
...
### we want to make sure all the species in ass_2010 are assigned a 1 for 
### ass (assessed) from 2011-onwards
...
landings_prop_df <- ass_catch %>%
  ...
  summarize(assessed_landings = sum(value, na.rm = TRUE)) %>%
  filter(assessed == 1) %>%            
    ### grab only the assessed landings data
  select(year, assessed_landings) %>% 
  full_join(yrly_catch) %>%
  mutate(prop_land_ass = assessed_landings/total_catch) 
    ### calculate the total proportion of landings that is assessed
