...
write_csv(saup_bc_sum, file.path(dir_goal, 'output/saup_catch_tot.csv'))
...
saup_bc_sum <- read_csv(file.path(dir_goal, 'output/saup_catch_tot.csv')) %>%
  left_join(read_csv(file.path(dir_goal, 'raw', 'ram_ids_to_names.csv')),
            by = c('ram_stockid' = 'stock_id'))

sum_by_ram_area <- saup_bc_sum %>%
  group_by(year, stock_name_mod, taxonkey, area_id, area_code) %>%
  summarize(catch = sum(rgn_catch)) %>%
  ungroup()

sum_by_ohibc_rgn <- saup_bc_sum %>%
  group_by(year, stock_name_mod, taxonkey, rgn_id) %>%
  summarize(catch = sum(rgn_catch)) %>%
  ungroup() %>%
  left_join(get_rgn_names(), by = 'rgn_id')


ram_rgn_plot <- ggplot(sum_by_ram_area, aes(x = stock_name_mod, y = catch, fill = area_code)) +
  ggtheme_plot(base_size = 7) +
  geom_bar(stat = 'identity') +
  coord_flip()

ggsave(file.path(dir_goal, 'Figs', 'saup_catch_by_ram_rgn.png'), width = 6, height = 6)

ohibc_rgn_plot <- ggplot(sum_by_ohibc_rgn, aes(x = stock_name_mod, y = catch, fill = rgn_name)) +
  ggtheme_plot(base_size = 7) +
  geom_bar(stat = 'identity') +
  coord_flip()

ggsave(file.path(dir_goal, 'Figs', 'saup_catch_by_ohibc_rgn.png'), width = 6, height = 6)
