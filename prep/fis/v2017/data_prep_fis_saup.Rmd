...
write_csv(rgn_to_saup, file.path(dir_goal, 'saup/rgn_to_saup.csv'))
...
saup_raw_file <- file.path(dir_anx, 'fis/v2017/saup/saup_bc_raw.csv')
...
Landings only, for Canada: 
year | taxon | allocation | sector type | reported/unreported/total

Landings only, for non-Canada: 
year | taxon | allocation | sector type | reported/unreported/total
...
saup_bc_sum <- saup_bc_raw %>%
  ...
  group_by(year,
           ...
           catch_type_name,
           reporting_status_name) %>%
  summarize(rgn_catch = sum(allocatedcatch * weight)) %>%
