...
stocks_exclude <- c('BIGEYE', 'BTUNA', 'MARLIN', 'SWORD', 'YFIN', 'SKJ')
  ### These get picked up from multinational managed stocks

bc_stocks <- bc_stocks %>%
  ### further refine georegions for multinationals:
  filter(str_detect(areaid, 'Canada') | str_detect(areaname, paste(rgn_keep, collapse = '|'))) %>%
  filter(!str_detect(stockid, paste(stocks_exclude, collapse = '|')))
