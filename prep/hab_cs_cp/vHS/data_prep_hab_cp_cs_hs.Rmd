...
seagrass_expos_vec <- lapply(as.list(stack_seagrass_expos), 
                FUN = function(x) {
...
seagrass_df <- data.frame(year = names(seagrass_expos_vec),
                    pres_expos_mean_sg = seagrass_expos_vec,
                    cp_score_sg = 1 - seagrass_expos_vec,
                    pres_mean_sg = seagrass_raw_pres_vec)

seagrass_df <- seagrass_df %>%
  mutate(sg_area_km2 = sum(!is.na(values(stack_seagrass_expos[[1]]))) * .01)
...
writeRaster(stack_cf_expos_elev, filename = cf_rast_files, bylayer = TRUE, overwrite = TRUE)
