---
title: 'OHIBC: Habitat goal (CP, CS, HAB) layers prep'
output: 
  html_document:
    code_folding: hide
    toc: true
...
if(any(!file.exists(landuse_files))) {
  ...
  stack_landuse <- stack(landuse_files_raw) %>%
    crop(rast_rgn)
  stack_landuse <- stack_landuse %>%
    mask(rast_rgn)
...
sm_exp_df2 <- sm_exp_df %>%
  ...
  summarize(area_km2_tot = sum(area_km2),
            ...
            sm_expos_area_mean = sm_expos_area_tot / area_km2_tot) %>%
  filter(!is.na(rgn_id)) %>%
  ungroup()

write_csv(sm_area_df2, file.path(dir_goal, 'int/sm_area_df.csv'))
write_csv(sm_exp_df2,  file.path(dir_goal, 'int/sm_exposure_df.csv'))
...
cf_exp_df2 <- cf_exp_df %>%
  ...
  summarize(area_km2_tot = sum(area_km2),
            ...
            cf_expos_area_mean = cf_expos_area_tot / area_km2_tot) %>%
  filter(!is.na(rgn_id)) %>%
  ungroup()

write_csv(cf_area_df2, file.path(dir_goal, 'int/cf_area_df.csv'))
write_csv(cf_exp_df2,  file.path(dir_goal, 'int/cf_exposure_df.csv'))
