...
See the data_prep_hab_cp_cs.Rmd file for data sources.
...
where $h$ represents a height adjustment factor.  In this assessment, $h$ will simply be held constant at 1 (for elevations below 5 m) or 0 (at elevations above 5 m).
...
cp_cf <- read_csv(file.path(dir_goal, 'int/forest_status_trend.csv')) %>%
...
cp_sm <- read_csv(file.path(dir_goal, 'int/saltmarsh_status_trend.csv')) %>%
...
# cp_sg <- read_csv(file.path(dir_goal, 'int/seagrass_status_trend.csv')) %>%
