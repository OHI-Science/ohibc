...
if(!file.exists(wdpa_bc_rast_file) | reload) {
  ...
  git_prov(wdpa_bc_shp_file,  filetype = 'input')
  git_prov(wdpa_bc_rast_file, filetype = 'output')
...
if(!file.exists(pep_bc_rast_file) | reload) {
  ...
  git_prov(pep_bc_shp_file,  filetype = 'input')
  git_prov(pep_bc_rast_file, filetype = 'output')
...
if(!file.exists(tribal_rast_file) | reload) {
  ...
  shp_tmp <- readOGR(dsn = dirname(tribal_shp_file),
                     layer = basename(tribal_shp_file) %>% str_replace('.shp$', ''))
  shp_tmp@data$tmp_id <- -2
  writeOGR(shp_tmp, path.expand(dir_goal), 'tmp_shp', overwrite = TRUE)

  rast_tribal  <- gdal_rast2(src = file.path(dir_goal, 'tmp_shp.shp'),
                          rast_base = rast_base, 
                          dst = tribal_rast_file, 
                          value = 'tmp_id', ### no year field available
                          override_p4s = TRUE)
  
  unlink(file.path(dir_goal, 'tmp_shp.*'))
  
} else {
  
  git_prov(tribal_shp_file,  filetype = 'input')
  git_prov(tribal_rast_file, filetype = 'output')
...
if(!file.exists(lsp_mapp_shp_file)) {
  mapps <- lapply(mapp_shp_files, 
                  FUN = function(x) {
                    sf::read_sf(dsn = dirname(x),
                                layer = basename(x) %>% str_replace('.shp$', ''))
                  }) %>%
  ...
  ### Convert to sp object for easier saving:
  mapp_mpas_sp <- as(st_zm(mapp_mpas), 'Spatial')
  ### st_zm() drops z and m dimensions
  
  rgdal::writeOGR(mapp_mpas_sp,
               dsn   = path.expand(mpa_mapp_shp_file) %>% dirname(),
               layer = basename(mpa_mapp_shp_file) %>% str_replace('.shp$', ''),
               driver = 'ESRI Shapefile',
               overwrite_layer = TRUE)  
  
  # x <- mapp_mpas %>%
  #   as.data.frame() %>%
  #   select(rgn, type, mgmt, obj) %>%
  #   distinct()
  # write_csv(x, file.path(dir_goal, 'int/mapp_management_zones.csv'))
  
  lsp_keywords <- c('tradition', 'cultur', 'historic', 'first n', 'recreat', 'haida')
  ...
  ### Convert to sp object for easier saving:
  mapp_lsp_sp <- as(st_zm(mapp_lsp), 'Spatial')
    ### st_zm() drops z and m dimensions

  
  rgdal::writeOGR(mapp_lsp_sp,
               dsn   = path.expand(lsp_mapp_shp_file) %>% dirname(),
               layer = basename(lsp_mapp_shp_file) %>% str_replace('.shp$', ''),
               driver = 'ESRI Shapefile',
               overwrite_layer = TRUE)
}
...
mapp_rast_file <- file.path(dir_rast, 'rast_mapp_500m.tif')

reload <- TRUE

if(!file.exists(mapp_rast_file) | reload) {
  
  rast_mapp  <- gdal_rast2(src = lsp_mapp_shp_file,
                          rast_base = rast_base, 
                          dst = mapp_rast_file, 
                          value = 'year',
                          override_p4s = TRUE)
  ...
  git_prov(lsp_mapp_shp_file,  filetype = 'input')
  git_prov(mapp_rast_file, filetype = 'output')
...
rast_mapp <- raster(file.path(dir_rast, 'rast_mapp_500m.tif'))
...
if(!file.exists(rast_prot_file) | reload) {
  rast_prot <- rast_wdpa
  ...
  values(rast_prot)[is.na(values(rast_prot))] <- values(rast_mapp)[is.na(values(rast_prot))]
  values(rast_prot)[is.na(values(rast_prot))] <- values(rast_tribal)[is.na(values(rast_prot))]
  values(rast_prot)[is.na(values(rast_prot))] <- values(rast_pep)[is.na(values(rast_prot))]
  
  writeRaster(rast_prot, file.path(dir_rast, 'rast_prot_500m.tif'), 
              overwrite = TRUE)
} else {
...
rast_prot   <- raster(file.path(dir_rast, 'rast_prot_500m.tif')) %>%
  crop(map_ext) %>%
  rasterToPoints() %>%
  data.frame()
...
ggplot() +
  ...
  geom_raster(data = rast_prot,   aes(x = x, y = y), fill = 'red', alpha = .25) +
  scale_fill_brewer(palette = 'Blues') +
  theme(axis.title = element_blank())

ggsave(file.path(dir_goal, 'figs', 'protected_area_map.png'), height = 5)
...
We save outputs for the following layers:
...
mpa_mapp_rast_file <- file.path(dir_rast, 'rast_mapp_mpa_500m.tif')

reload <- TRUE

if(!file.exists(mpa_mapp_rast_file) | reload) {
  
  rast_mapp  <- gdal_rast2(src = mpa_mapp_shp_file,
                          rast_base = rast_base, 
                          dst = mpa_mapp_rast_file, 
                          value = 'year',
                          override_p4s = TRUE)
} else {
...
if(!file.exists(rast_mpa_file) | reload) {
  rast_mpa <- rast_wdpa
  
  ### The logic here: all rasters should line up.  An NA cell in the top-level
  ### raster gets replaced by the matching cell in the next-level raster,
  ### whether that's an NA or a new value.  And so on.
  values(rast_mpa)[is.na(values(rast_mpa))] <- values(mpa_rast_mapp)[is.na(values(rast_mpa))]
  values(rast_mpa)[is.na(values(rast_mpa))] <- values(rast_tribal)[is.na(values(rast_mpa))]
  values(rast_mpa)[is.na(values(rast_mpa))] <- values(rast_pep)[is.na(values(rast_mpa))]
  
  writeRaster(rast_mpa, rast_mpa_file, 
              overwrite = TRUE)
} else {
  git_prov(rast_mpa_file, filetype = 'output')
}
```
...
rast_3nm_file    <- file.path(dir_spatial, 'raster/ohibc_offshore_3nm_raster_500m.tif')
rast_eez_file    <- file.path(dir_spatial, 'raster/ohibc_rgn_raster_500m.tif')
rast_mpa_file   <- file.path(dir_rast, 'rast_mpa_500m.tif')

rast_3nm <- raster(rast_3nm_file)
rast_eez <- raster(rast_eez_file)
...
ptm <- proc.time()
stats_3nm_mpa <- raster::crosstab(rast_mpa, rast_3nm, useNA = TRUE, progress = 'text') %>%
  as.data.frame() %>%
  setNames(c('year', 'rgn_id', 'n_cells')) %>%
  mutate(year   = as.integer(as.character(year)),
         rgn_id = as.integer(as.character(rgn_id))) %>%
...
stats_eez_mpa <- raster::crosstab(rast_mpa, rast_eez, useNA = TRUE, progress = 'text') %>%
  as.data.frame() %>%
  setNames(c('year', 'rgn_id', 'n_cells')) %>%
  mutate(year   = as.integer(as.character(year)),
         rgn_id = as.integer(as.character(rgn_id))) %>%
...
mpa_mgmt_file <- file.path(dir_anx, '_raw_data/fisheries_resilience/fisheries_resilience.xlsx')
mpa_mgmt_plans <- readxl::read_excel(mpa_mgmt_file, sheet = 'ProtectAreaPlans') %>%
