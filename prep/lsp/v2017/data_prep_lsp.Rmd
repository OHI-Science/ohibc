...
if(!file.exists(lsp_mapp_shp_file)) {
  ...
  mapp_nc@data <- mapp_nc@data %>%
    select(rgn = SubRegion, type = Type, mgmt = Management, 
           obj = Objective, name = Name) 
  mapp_nc <- mapp_nc[(is.na(mapp_nc@data$mgmt)) |
                     !str_detect(tolower(mapp_nc@data$mgmt), 
                                 'renewable|aquaculture|future planning'), ]
  
  mapp_hg <- mapps[['HaidaGwaii']]
  mapp_hg@data <- mapp_hg@data %>%
    select(rgn = SubRegion, type = Type, mgmt = Management, 
           obj = Objective, name = Name)
  mapp_hg <- mapp_hg[(is.na(mapp_hg@data$mgmt)) |
                     !str_detect(tolower(mapp_hg@data$mgmt), 
                                 'alt.energy|shellfish aq'), ]
  
  mapp_cc <- mapps[['CentralCoast']]
  mapp_cc@data <- mapp_cc@data %>%
    select(rgn = SubRegion, type = Type, mgmt = ZoneType, 
           obj = AreaDecri, name = Group_)
  mapp_cc <- mapp_cc[(is.na(mapp_cc@data$mgmt)) |
                     !str_detect(tolower(mapp_cc@data$mgmt), 
                                 'renewable|aquaculture'), ]
  
  mapp_nvi <- mapps[['NorthVancouverIsland']]
  mapp_nvi@data <- mapp_nvi@data %>%
    select(rgn = SubRegion, type = Type, mgmt = Management, 
           obj = AreaDescri, name = Name)
  mapp_nvi <- mapp_nvi[(is.na(mapp_nvi@data$mgmt)) |
                     !str_detect(tolower(mapp_nvi@data$mgmt), 
                                 'renewable|aquaculture'), ]
  
  all_mapp <- rbind(mapp_nc, mapp_hg, mapp_cc, mapp_nvi)
  ...
  mapp_lsp <- all_mapp[str_detect(tolower(all_mapp@data$obj), paste(lsp_keywords, collapse = '|')), ]
  mapp_lsp@data <- mapp_lsp@data %>%
    mutate(year = 2015)
  
  rgdal::writeOGR(mapp_lsp,
