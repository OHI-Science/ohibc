...
rast_lsp_file <- file.path(dir_rast, 'rast_lsp_500m.tif')
...
writeRaster(rast_lsp, rast_lsp_file, 
            overwrite = TRUE)
...
rast_lsp   <- raster(file.path(dir_rast, 'rast_lsp_500m.tif')) %>%
  crop(map_ext) %>%
  rasterToPoints() %>%
  data.frame()
...
zonal_3nm_file    <- file.path(dir_goal, 'int', 'zonal_stats_lsp_3nm.csv')
zonal_inland_file <- file.path(dir_goal, 'int', 'zonal_stats_lsp_inland.csv')
