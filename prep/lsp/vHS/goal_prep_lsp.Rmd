...
If the WDPA-MPA shapefile has already been rasterized for the global assessment, then we may be able to use that directly.  It is Mollweide projection which is not ideal for BC, as we would prefer to work in BC Albers.  Instead, we will create a BC-specific polygon subset, then rasterize to BC Albers.
...
`r knitr::kable(prot_3nm %>% filter(year == max(year)) %>% select(-contains('cell')), caption = '3 nautical mile offshore zone - area in km^2^')`

`r knitr::kable(prot_1km %>% filter(year == max(year)) %>% select(-contains('cell')), caption = '1 kilometer inland zone - area in km^2^')`
