...
## Set up coastal buffers and base raster
...
### provenance check:
git_prov(sprintf('%s/%s.shp', path.expand(dir_rgn), lyr_3nm))
git_prov(sprintf('%s/%s.shp', path.expand(dir_rgn), lyr_1km))
...
rast_cols = (colorRampPalette(RColorBrewer::brewer.pal(9, 'Reds'))(255)) # rainbow color scheme
...
## Extract WDPA protected areas against buffer regions
...
The result is a dataframe that sums the protected areas within each coastal region (3 nm offshore and 1 km inland, separately)
...
## Calculate status for each region for each year

From the sums, determine the ratio of the protected area to the total area within the regions; compare to reference point of 30%; and then take mean of inland score and offshore score. 
...
prot_mar_df <- prot_mar_df %>%
  mutate(ratio = (cum_a_km2/tot_a_km2)/ref_pt*100,
         status = ifelse(ratio > 100, 100, ratio))
...
write_csv(lsp_status, file.path(dir_lsp, 'data/status.csv'))
git_prov(file.path(dir_lsp, 'data/status.csv'), type = 'output')
