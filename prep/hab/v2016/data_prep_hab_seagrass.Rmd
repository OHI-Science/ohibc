...
rast_base_file <- file.path(dir_spatial, 'ohibc_rgn_raster_100m.tif')
...
new_rast_files <- file.path(dir_goal_anx, 'tif', basename(nutr_files)) %>%
...
if(any(!file.exists(new_rast_files))) {
  ...
  tmp <- mclapply(nutr_files, mc.cores = 12, FUN = function(x) {
    new_rast_file <- file.path(dir_goal_anx, 'tif', basename(x)) %>%
      str_replace('global', 'bc')
    old_rast <- raster(x)
    raster::projectRaster(old_rast, rast_base, 
...
- Rasterize buffer to 0.1 km and summarize area per region
- this raster will be stored on git-annex

``` {r create_bluecarbon_seagrass_raster, eval = TRUE}

seagrass_rast_file <- file.path(dir_goal_anx, 'tif/bluecarbon_seagrass.tif') %>%
  path.expand()
...
After creating buffer shapefiles and rasters, tabulate seagrass area per OHIBC region. This CSV stored in github.
...
if(!file.exists(seagrass_rgns_file)) {
  seagrass_rast <- raster(file.path(dir_goal_anx, 'tif', 'bluecarbon_seagrass.tif')) %>%
    setNames('seagrass')
  ...
  git_prov(file.path(dir_goal_anx, 'tif', 'bluecarbon_seagrass.tif'), filetype = 'input')
...
nutr_layers <- list.files(file.path(dir_goal_anx, 'tif'), 
                          pattern = '*_fert_*', 
                          full.names = TRUE)

outfile_names <- file.path(dir_goal_anx, 'tif', 
...
nutr_layer_files <- list.files(file.path(dir_goal_anx, 'tif'), 
...
if(!file.exists(seagrass_nutr_df_file)) {
  ...
  # library(parallel)
  # sg_nutr_df_list <- mclapply(seagrass_nutr_list, mc.cores = 12,
  #   FUN = function(sg_nutr_rast) { ### sg_nutr_rast <- seagrass_nutr_list[[1]]
  #     nutr_year <- names(sg_nutr_rast) %>%
  #       str_replace('seagrass_nutr_', '')
  #     
  #     nutr_rast <- sg_nutr_rast %>%
  ...
  for (i in 1:length(seagrass_nutr_list)) { # i <- 1
    ...
    nutr_year <- names(seagrass_nutr_list[[i]]) %>%
      str_replace('seagrass_nutr_', '')
    message('cross tabulating nutrient pressures for ', nutr_year)
