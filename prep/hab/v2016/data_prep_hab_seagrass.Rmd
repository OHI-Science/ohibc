...
``` {r read and clip pressure raster}
...
``` {r create_seagrass_buffer_raster}
...
seagrass_rast <- raster(file.path(dir_goal, 'tif', 'buffer_seagrass.tif')) %>%
  setNames('seagrass')

rgn_rast <- raster(file.path(dir_spatial, 'ohibc_rgn_raster_500m.tif')) %>%
  setNames('rgn_id')

sg_buffer_df <- raster::crosstab(seagrass_rast, rgn_rast, 
...
write_csv(sg_buffer_df, file.path(dir_goal, 'int', 'seagrass_bioband_area.csv'))
