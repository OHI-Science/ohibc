...
zonal_05_file <- file.path(dir_goal, 'int', 'zonal_stats_2005_10km.csv')
zonal_10_file <- file.path(dir_goal, 'int', 'zonal_stats_2010_10km.csv')
...
if(!file.exists(zonal_05_file) | !file.exists(zonal_10_file)) {
  ...
  ptm <- proc.time()
  stats_05 <- raster::crosstab(rast_lc_05_bcalb, rast_10km_100m, useNA = TRUE, progress = 'text') %>%
    as.data.frame() %>%
  ...
  ptm <- proc.time()
  stats_10 <- raster::crosstab(rast_lc_10_bcalb, rast_10km_100m, useNA = TRUE, progress = 'text') %>%
    as.data.frame() %>%
    setNames(c('value', 'rgn_id', 'n_cells')) %>%
    mutate(value   = as.integer(as.character(value)),
           rgn_id = as.integer(as.character(rgn_id))) %>%
...
write_csv(wetland_df, file.path(dir_goal, 'int', 'wetland_stats_10km.csv'))

```

``` {r expand time series}

wetland_df <- read_csv(file.path(dir_goal, 'int', 'wetland_stats_10km.csv'))
...
write_csv(wetland_ts, file.path(dir_goal, 'int', 'wetland_time_series_10km.csv'))
