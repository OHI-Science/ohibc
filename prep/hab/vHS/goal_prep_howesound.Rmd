...
***QUESTION*** do we weight the habitats by reference area, or by current area? not an issue in global.
...
cp_scores_summary <- cp_scores %>%
  ...
  mutate(cp_st_tot = sum(score * hab_weight * area) / sum(hab_weight * area),
         cp_st_tot_ref = sum(score * hab_weight * a_ref) / sum(hab_weight * a_ref),
         cp_tr_tot = sum(trend * hab_weight * area) / sum(hab_weight * area),
         cp_tr_tot_ref = sum(trend * hab_weight * a_ref) / sum(hab_weight * a_ref)) %>%
...
cs_scores_summary <- cs_scores %>%
  ...
  mutate(a_ref = first(area)) %>%
  ungroup() %>% 
  mutate(cs_st_tot = sum(c_cref * hab_weight * area) / sum(hab_weight * area),
         cs_tr_tot = sum(trend * hab_weight * area) / sum(hab_weight * area),
         cs_st_tot_ref = sum(c_cref * hab_weight * a_ref) / sum(hab_weight * a_ref),
         cs_tr_tot_ref = sum(trend * hab_weight * a_ref) / sum(hab_weight * a_ref))
