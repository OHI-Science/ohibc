...
# 2         Etrumeus teres  Round herring
# 3  Opisthonema libertate thread herring
# 4 Opisthonema medirastre thread herring ### This one is ranked as LC in IUCN but only as far north as central CA
...
# 2         Etrumeus teres  Round herring
# 3  Opisthonema libertate thread herring
# 4 Opisthonema medirastre thread herring
...
ico_table_info <- ico_table_info %>%
  ...
  group_by(spp_health) %>%
  mutate(cat_lab = ifelse(n_ico == last(n_ico), cat_lab, NA)) %>%
  mutate(ico_lab = paste0(comname, ' (', sciname, ')'),
         ico_lab = ifelse(!is.na(pr_score), ico_lab, paste0(ico_lab, '*')))

ico_plot <- ggplot(ico_table_info, aes(x = n_ico, y = 0.3, fill = spp_health)) +
  ggtheme_plot + 
