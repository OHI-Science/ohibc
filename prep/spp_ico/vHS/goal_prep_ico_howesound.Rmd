...
knitr::kable(ico_status_ts, caption = 'Howe Sound iconic species status and trend')
...
ico_table_info <- ico_info_timeseries %>%
  ...
  mutate(spp_health = 1 - cat_score,
         n_ico = 1:n())
  
ico_plot <- ggplot(ico_table_info, aes(x = n_ico, y = 0.2, fill = spp_health)) +
  ggtheme_plot + 
  theme(panel.grid = element_blank(),
        axis.title = element_blank(),
        axis.text  = element_blank()) +
  geom_bar(stat = 'identity', width = 1) +
  geom_text(aes(label = sciname, nudge_y = .02), 
            size = 2,
            # angle = 90, 
            hjust = 0) +
