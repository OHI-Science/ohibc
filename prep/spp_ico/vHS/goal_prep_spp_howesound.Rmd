...
spp_info <- read_csv(spp_global_file) %>%
  mutate(sciname = ifelse(am_sid == 'Fis-153539', 'Clupea pallasii', sciname)) 
  ### force Pacific herring into a name recognized on the BC Species Explorer list

spp_info <- spp_append_bcsee(spp_info)

iucn_cells_spp <- read_csv(file.path(dir_goal, 'int', 'iucn_cells_hs_spp.csv'))
am_cells_spp   <- read_csv(file.path(dir_goal, 'int', 'am_cells_hs_spp.csv'))
...
### add HS specific species to the list
spp_info_hs <- spp_info_hs %>%
  bind_rows(read_csv(file.path(dir_goal, 'int/spp_info_hs_specific.csv')))
...
spp_past_df_hs <- spp_past_df_hs %>%
  ...
  arrange(iucn_sid, year) %>%
  group_by(iucn_sid) %>%
  fill(cat_ts, cat_txt, cat_ts_score) %>% ### fills all the way to latest year (2016)
  ungroup()

spp_clean <- read_csv(file.path(dir_goal, 'int/spp_info_clean.csv'))

spp_clean1 <- spp_clean %>% 
  full_join(spp_past_df_hs, by = 'iucn_sid')
...
spp_clean1 <- spp_clean1 %>%
  mutate(year = ifelse(is.na(year), list(c(1995:2016)), year)) %>% 
  unnest(year) %>%
  filter(year >= 1995)
...
spp_clean1 <- spp_clean1 %>%
  mutate(cat_ts_score = ifelse(year == 2016 & is.na(cat_ts), cat_score, cat_ts_score)) %>%
  arrange(iucn_sid, year) %>%
  group_by(iucn_sid) %>%
...
The resulting dataframe, most recent year only: 

`r DT::datatable(spp_clean1 %>% filter(year == 2016) %>% select(-year))`
