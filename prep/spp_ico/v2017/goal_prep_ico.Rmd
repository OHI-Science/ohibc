...
# Summary: OHIBC Species and Iconic Species
...
Using OHIBC region polygon, determine 0.5° raster cells corresponding to the region.
...
ico_info_ts1 <- read_csv(file.path(dir_goal, 'int/ico_info_timeseries.csv')) %>%
  ...
  select(sciname, comname, iucn_sid, am_sid, 
         ...
         risk_trend, cat_ts_score, year) %>%
  distinct()

am_ico_rgn <- read_csv(file.path(dir_goal, 'int/am_spp_cells.csv')) %>%
  select(am_sid, rgn_id) %>%
  distinct()
iucn_ico_rgn <- read_csv(file.path(dir_goal, 'int/iucn_spp_cells.csv')) %>%
...
ico_rgn_sum_ts <- ico_info_rgn_ts %>%
  select(rgn_id, sciname, cat_ts_score, risk_trend, year) %>%
  group_by(rgn_id, year) %>%
  summarize(mean_cat   = mean(cat_ts_score, na.rm = TRUE),
            mean_trend = mean(risk_trend, na.rm = TRUE),
            n_total    = n(),
            n_cat      = sum(!is.na(cat_ts_score)),
            n_trend    = sum(!is.na(risk_trend)),
            status     = 1 - mean_cat,
            trend      = -(mean_trend * 5) / status)

write_csv(ico_rgn_sum_ts %>%
            select(rgn_id, score = status, year), 
          file.path(dir_goal, 'output/ico_status.csv'))
write_csv(ico_rgn_sum_ts %>%
            select(rgn_id, score = trend, year), 
          file.path(dir_goal, 'output/ico_trend.csv'))
write_csv(ico_rgn_sum_ts, 
          file.path(dir_goal, 'summary/ico_rgn_summary.csv'))

DT::datatable(ico_rgn_sum_ts %>% arrange(desc(year)), 
             caption = 'OHIBC iconic species status and trend')
