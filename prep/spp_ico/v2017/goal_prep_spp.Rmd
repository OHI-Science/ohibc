...
spp_bc_file <- file.path(dir_goal, 'int/spp_list_ohibc.csv')
...
assess_years <- c(2000:2017)
trend_span   <- 10

spp_cat_trend_list <- vector('list', length = length(assess_years))

for(i in 1:length(assess_years)) { # i <- 1
  
  yr <- assess_years[i]
  spp_cat_current <- spp_cat_ts %>%
    filter(year < yr & year >= yr - trend_span)

  spp_cat_trend_list[[i]] <- spp_cat_current %>%
    group_by(iucn_sid) %>%
    do(risk_trend = lm(cat_ts_score ~ year, data = .)$coefficients['year']) %>%
    left_join(spp_cat_current %>% 
                filter(year == yr - 1), ### e.g. 2017 assessment looks at 2016 category
              by = 'iucn_sid') %>%
    mutate(risk_trend = round(risk_trend, 4),
           assess_year = yr)
}

spp_cat_trend_df <- bind_rows(spp_cat_trend_list)

write_csv(spp_cat_trend_df, file.path(dir_goal, 'int/spp_cat_trend_timeseries.csv'))
...
spp_cat_ts <- read_csv(file.path(dir_goal, 'int', 'spp_cat_trend_timeseries.csv'))
...
spp_scored <- spp_scored %>%
  ...
  filter(!(is.na(cat_ts_score) & year != max(year)))
...
write_csv(am_cells_sum, file.path(dir_goal, 'summary/am_cells_summary.csv'))
...
write_csv(iucn_cells_sum, file.path(dir_goal, 'summary/iucn_cells_summary.csv'))

```

### Map of species count by cell

``` {r spp_plot_species_count_raster, fig.height = 4, fig.width = 6, fig.align = 'center'}
...
iucn_cells_sum <- read_csv(file.path(dir_goal, 'summary/iucn_cells_summary.csv'))
am_cells_sum   <- read_csv(file.path(dir_goal, 'summary/am_cells_summary.csv'))
rgn2cell_df    <- read_csv(file.path(dir_goal, 'spatial/rgn2cell.csv'))
...
sum_by_rgn     <- spp_calc_rgn_means(sum_by_loiczid, 
                                     rgn2cell_df, 
                                     rgn_tag = '')
...
Region summary saved to: `r file.path(dir_goal, 'summary/rgn_summary.csv')`

`r DT::datatable(sum_by_rgn, caption = 'Region summary (all years)')`
...
sum_by_rgn <- read_csv(file.path(dir_goal, 'summary/rgn_summary.csv'))
...
``` {r calc_3nm_SPP_status_and_trend}
...
sum_by_loiczid <- read_csv(file.path(dir_goal, 'summary/cell_spp_summary_by_loiczid.csv'))

sum_by_rgn_3nm  <- spp_calc_rgn_means(sum_by_loiczid, rgn2cell_3nm_df, rgn_tag = '_3nm')
...
3 Nautical Mile region summary saved to: `r file.path(dir_goal, 'summary/rgn_summary_3nm.csv')`
