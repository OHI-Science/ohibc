...
spp_cat_ts_layer <- spp_list_scored %>%
  select(iucn_sid, am_sid, cat_ts_score, year, pr_score, cat_ts) %>%
  distinct()
write_csv(spp_cat_ts_layer, file.path(dir_goal, 'output', 'spp_cat_by_year.csv'))
...
spp_cat_by_yr <- read_csv(file.path(dir_goal, 'output', 'spp_cat_by_year.csv')) %>%
  filter(!is.na(year)) %>%
  complete(year = full_seq(year, period = 1), nesting(iucn_sid, am_sid)) %>%
  group_by(iucn_sid, am_sid) %>%
  arrange(year) %>%
  fill(cat_ts_score, .direction = 'up') %>%
  fill(cat_ts_score, .direction = 'down') %>%
  filter(year >= 1990) %>%
  select(iucn_sid, am_sid, year, cat_ts_score) %>%
  ungroup()

spp_cat_by_rgn <- spp_area_by_rgn %>%
  inner_join(spp_cat_by_yr, by = c('iucn_sid', 'am_sid')) %>%
  filter(!is.na(cat_ts_score))

spp_cat_rgn_yr <- spp_cat_by_rgn %>%
  group_by(rgn_id, year) %>%
  summarize(mean_cat_score = sum(cat_ts_score * spp_pct_area) / n()) %>%
  mutate(status = (.75 - mean_cat_score) / .75,
         status = round(status, 5),
         mean_cat_score = round(mean_cat_score, 5))
...
spp_cat_3nm <- spp_3nm_area %>%
  full_join(spp_cat_by_yr, by = c('iucn_sid', 'am_sid')) %>%
  filter(!is.na(cat_ts_score))
