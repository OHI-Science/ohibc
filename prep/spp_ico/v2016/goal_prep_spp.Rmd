...
Variables in `spp_info`: `r paste(names(spp_info), collapse = ' | ')`

``` {r print_global_species_list, eval = TRUE}
knitr::kable(head(spp_info %>% 
                dplyr::select(sciname, am_sid, iucn_sid, pop_trend, pop_cat, spatial_source)),
             align = c('l', 'c', 'c', 'c', 'c', 'c'),
             caption = 'Quick glance at global spp_info dataframe:')
```
...
write_csv(spp_info, file.path(dir_goal, 'int/spp_info_raw.csv'))
...
if(!file.exists(spp_cat_ts_file)) {
  ...
  spp_past_df <- rbind_all(spp_past_list)
