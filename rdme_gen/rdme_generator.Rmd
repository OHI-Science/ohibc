...
readme_test <- dir_sum_df %>%
  filter(str_detect(dir, '/prep/') & !str_detect(dir, '/_spatial')) %>%
...
for(i in seq_along(readme_test$dir)) {
  ### Check whether prep, year, or generic
  # i <- 33

  prep_dir <- readme_test$dir[i]
  
  message('Creating readme_test.md for ', prep_dir)
  
  readme_type <- case_when(str_detect(basename(prep_dir), 'v[0-9]{4}') ~ 'generic_readme_year.md',
                           str_detect(prep_dir, '/prep/[a-zA-Z]{2}')   ~ 'generic_readme_prep.md',
                           TRUE ~ 'generic_readme.md')
  
  readme_text <- read_file(file.path(dir_git, 'src/templates', readme_type)) %>%
    str_replace('_DIRNAME_', readme_test$dir_short[i]) %>%
    str_replace('_SUMMARYINFO_', readme_test$text_block_all[i])
  
  readr::write_file(readme_text, file.path(readme_test$dir[i], 'readme_test.md'))
  
}

# x <- list.files(dir_git, recursive = TRUE, pattern = 'readme_test.md', full.names = TRUE)
# unlink(x)
```
