...
get_rmd_info <- function(rmd_file) {
  ...
  rmd_sum <- rmd_text %>% 
    ...
    as.data.frame() %>%
    setNames('text') %>%
    filter(str_detect(tolower(text), 'summary')) %>%
...
rmd_test <- dir_sum_df %>%
  ...
  mutate(text_block = sprintf('### %s:\n\n* __Summary:__ %s * __Rmd file:__ %s * __HTML file:__ %s',
                              readme_title, rmd_summary, 
                              file.path(github_url, rmd), 
                              file.path(rawgit_url, html))) %>%
  group_by(dir) %>%
  summarize(text_block_all = paste(text_block, collapse = '\n\n')) %>%
  ungroup() %>%
  mutate(dir_short = str_replace(dir[1], path.expand(dir_git), ''))

### Check whether prep, year, or generic
rmd_dir <- rmd_test$dir

readme_type <- case_when(str_detect(basename(rmd_dir), 'v[0-9]{4}') ~ 'generic_readme_year.md',
